...
if [ -n "$CI" ]; then
  ...
  # and doing less work.
  # we achieve this by setting the SALTO_TEST_HASH env var, which turbo.json uses in 
  ...
  if [ -n "$filter" ]; then
    ...
    export SALTO_TEST_HASH="$(node ./build_utils/hash_dependencies.js -p "$filter")"
  else
    echo "node ./build_utils/hash_dependencies.js"
    export SALTO_TEST_HASH="$(node ./build_utils/hash_dependencies.js)"
  fi
