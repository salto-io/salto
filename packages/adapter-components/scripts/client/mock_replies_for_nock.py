...
def convert_to_nock_format(line):
    http_data = json.loads(line)
    http_data['scope'] = ''
    http_data['path'] = http_data.pop('url')
    http_data['body'] = http_data.pop('data')
    http_data['reqHeaders'] = http_data.pop('headers')
    return json.dumps(http_data)

def to_mock_details(logpath):
  with open(logpath) as logfile:
    return [convert_to_nock_format(l[l.find(': ') + 2:].strip()) for l in logfile.readlines() if 'Full HTTP response' in l]
