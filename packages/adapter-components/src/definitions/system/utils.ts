...
export const queryWithDefault = <T, K extends string = string>(
  ...
  const query: DefQuery<T, K>['query'] = key =>
    mergeSingleDefWithDefault(
      ...
      defsWithDefault.customizations ? defsWithDefault.customizations[key] : undefined,
    )
  return {
    query,
  ...
  return _.merge({}, fetchConfig, {
    instances: {
      customizations: _.mapValues(userElemID, ({ extendSystemPartsDefinition, ...userDef }, type) => {
        const customizations = fetchConfig?.instances.customizations ?? {}
        const { elemID: systemDef } = customizations[type]?.element?.topLevel ?? {}
        const elemIDDef = {
