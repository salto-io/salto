...
import { StandaloneFieldConfigType, TransformationConfig, TransformationDefaultConfig } from '../../config_deprecated'
...
const convertStringToObject = (inst: InstanceElement, standaloneFields: StandaloneFieldConfigType[]): void => {
    ...
    if (standaloneFieldDef !== undefined) {
      try {
        ...
        if (shouldParseJson) {
          ...
          if (fieldValue === 'null') {
            log.info(
              'treating null as undefined in instance %s %s, value: %s',
          ...
          log.warn('not parsing json from instance %s %s, value: %s', inst.elemID.getFullName(), fieldName, fieldValue)
        }
        return fieldValue
      } catch (e) {
        log.error(
          'failed to convert field %s to JSON. Error: %s, value: %o, stack: %o',
          fieldName,
          e,
          fieldValue,
          e.stack,
...
const addFieldTypeAndInstances = async ({
  ...
  if (type.fields[fieldName] === undefined) {
    log.info(
      'type %s field %s does not exist (maybe it is not populated by any of the instances), not extracting field',
      type.elemID.name,
      fieldName,
    )
    return []
  ...
  const getInnerType = async (elem: TypeElement): Promise<TypeElement> =>
    isListType(elem) ? elem.getInnerType() : elem
  ...
  if (!isObjectType(fieldType)) {
    // should not happen
    log.warn(
      'expected field type %s to be an object type, not extracting standalone fields',
  ...
  await awu(instancesWithValues).forEach(async inst => {
    const fieldInstances = await awu(makeArray(inst.value[fieldName]))
      .map(async (val, index) => {
        const fieldInstance = await toInstance({
          ...
          transformationDefaultConfig,
          getElemIdFunc,
          nestedPath: [...(inst.path?.slice(2, inst.path?.length - 1) ?? []), fieldName],
        })
...
export const extractStandaloneFields = async ({
  ...
  await awu(Object.entries(typesWithStandaloneFields)).forEach(async ([typeName, standaloneFields]) => {
    ...
    await awu(standaloneFields).forEach(async fieldDef => {
      elements.push(
        ...(await addFieldTypeAndInstances({
          adapterName,
          typeName,
