...
import {
  FieldToHideType,
  FieldTypeOverrideType,
  getTypeTransformationConfig,
} from '../config_deprecated/transformation'
import { SUBTYPES_PATH, TYPES_PATH } from '../fetch/element/constants'
import { TypeConfig, TypeDefaultsConfig } from '../config_deprecated/shared'
 ...
 */
export const hideFields = (fieldsToHide: FieldToHideType[], type: ObjectType): void => {
  ...
  fieldsToHide.forEach(({ fieldName, fieldType }) => {
    if (fieldType !== undefined && fieldType !== typeFields[fieldName]?.refType.elemID.name) {
      if (typeFields[fieldName] === undefined) {
        ...
        log.warn(
          `Failed to hide field ${type.elemID.name}.${fieldName} - override type is ${fieldType} while type is ${typeFields[fieldName].refType.elemID.name}`,
        )
      }
      return
    }
...
export const filterTypes = async (
  ...
  const relevantTypes = typesToFilter
    .map(name => {
  ...
  relevantTypes
    ...
    .forEach(t => {
      t.path = [adapterName, TYPES_PATH, t.elemID.name]
    })
  ...
  subtypes
    .filter(t => t.path === undefined)
    .forEach(t => {
      t.path = [adapterName, TYPES_PATH, SUBTYPES_PATH, t.elemID.name]
    })
...
export const fixFieldTypes = (
  ...
  Object.keys(typeConfig)
    .filter(
      typeName => getTypeTransformationConfig(typeName, typeConfig, typeDefaultConfig).fieldTypeOverrides !== undefined,
    )
    .forEach(typeName => {
      ...
      const fieldTypeOverrides = getTypeTransformationConfig(typeName, typeConfig, typeDefaultConfig)
