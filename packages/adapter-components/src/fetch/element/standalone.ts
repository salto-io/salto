...
const extractStandaloneInstancesFromField = ({
  defQuery, instanceOutput, getElemIdFunc, parent,
}: {
  ...
  const newInstances = collections.array.makeArray(value).map((entry, index) => createInstance({
    ...
    nestUnderPath: standaloneDef.nestPathUnderParent
      ? [
        ...parent.path?.slice(2, parent.path?.length - 1) ?? [],
        field.name,
      ]
      : undefined,
  }))
 ...
 */
export const extractStandaloneInstances = ({ instances, defQuery, getElemIdFunc }: {
