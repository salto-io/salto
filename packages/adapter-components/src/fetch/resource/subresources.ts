...
export const recurseIntoSubresources = ({ def, typeFetcherCreator, contextResources }: {
    ...
    Object.entries(def.recurseInto ?? {})
      ...
      .map(async ([fieldName, recurseDef]) => {
        ...
        if (recurseDef.single) {
          ...
          log.warn(`Expected a single value in recurseInto result for ${recurseDef.typeName}.${fieldName} but received: ${items?.length ?? 0}, keeping as list`)
        }
