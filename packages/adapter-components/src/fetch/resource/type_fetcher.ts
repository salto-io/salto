...
const calculateContextArgs = ({ contextDef, initialRequestContext, contextResources }: {
  ...
  return _.defaults(
    ...
    _(remainingDependsOnArgs)
      .mapValues(arg => (contextResources[arg.parentTypeName]
        ?.flatMap(item =>
...
export const createTypeResourceFetcher = <ClientOptions extends string>({
  ...
  const getItems: TypeResourceFetcher['getItems'] = () => {
    ...
    if (validItems.length < items.length) {
      log.warn('[%s] omitted %d items of type %s that were not plain objects', adapterName, items.length - validItems.length, typeName)
    }
