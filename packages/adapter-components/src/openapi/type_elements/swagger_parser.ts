...
import { loadSwagger, LoadedSwagger } from '../load'
...
export const swaggerTypeToPrimitiveType = (val: string | string[] | undefined): PrimitiveType => {
...
export const isReferenceObject = (value: any): value is ReferenceObject => value?.$ref !== undefined
...
const toSchemaV2 = (def?: V2Def): undefined | OpenAPIV2.Schema => def?.schema

type V3Def = OpenAPIV3.ReferenceObject | OpenAPIV3.ResponseObject | OpenAPIV3.RequestBodyObject

const toSchemaV3 = (def?: V3Def): undefined | OpenAPIV3.SchemaObject | OpenAPIV3.ReferenceObject => {
...
const getSwaggerVersion = (swagger: LoadedSwagger): SwaggerVersion => {
  if (!(isV2(swagger.document) || isV3(swagger.document))) {
    ...
    throw new Error(
      `unsupported swagger version ${_.get(swagger.document, 'swagger') ?? _.get(swagger.document, 'openapi')}`,
    )
  }
...
export const toSchema = (swaggerVersion: SwaggerVersion, def?: V3Def | V2Def): SchemaOrReference | undefined =>
...
export const getParsedDefs = async ({
  ...
  const additionalSchemas = isV3(swagger.document)
    ? _.pickBy(swagger.document.components?.schemas, (_value, key) => additionalTypes?.has(key))
    : _.pickBy(swagger.document.definitions, (_value, key) => additionalTypes?.has(key))
  return {
...
export const extractProperties = (
  schemaDefObj: SchemaObject,
  refs: SwaggerRefs,
): {
  allProperties: Record<string, SchemaObject>
  additionalProperties?: SchemaObject
} => {
  const recursiveXOf = ({ allOf, anyOf, oneOf }: HasXOf): SchemaOrReference[] =>
    [allOf, anyOf, oneOf]
      .filter(isDefined)
      .flatMap(xOf => [...xOf, ...xOf.flatMap(nested => (isReferenceObject(nested) ? [nested] : recursiveXOf(nested)))])
    ...
    [
      ...
      ...recursiveXOf(schemaDef).flatMap(nested =>
        isReferenceObject(nested)
          ...
          : [nested.additionalProperties, ...flattenXOfAdditionalProps(nested.properties ?? {})],
      ),
    ]
      .filter(p => p !== false)
      .map(p => {
  ...
  const hasNonEmptyDefinition = (s: SchemaObject): boolean =>
    s.properties !== undefined || !_.isEmpty(s.type) || !_.isEmpty(s.allOf)
