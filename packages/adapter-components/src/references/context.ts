...
export const neighborContextGetter =
  ...
  async ({ instance, elemByElemID, fieldPath }) => {
    ...
    const resolveReference = async (context: ReferenceExpression, path?: ElemID): Promise<string | undefined> => {
    ...
    try {
      const parent =
        levelsUp === 'top' ? fieldPath.createTopLevelParentID().parent : findParentPath(fieldPath, levelsUp)
      ...
      const contextStr = isReferenceExpression(context) ? await resolveReference(context, contextPath) : context
      ...
      log.error(
        'could not resolve context for reference. error: %s, params: %s, stack: %s',
        e,
        { fieldPath, contextFieldName, levelsUp },
        e.stack,
      )
      return undefined
    }
