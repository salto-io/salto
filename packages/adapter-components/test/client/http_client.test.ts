...
describe('client_http_client', () => {
  ...
  describe('getSingle', () => {
    ...
    it('should make the right request', async () => {
      ...
      const getRes = await client.getSingle({ url: '/ep' })
      const getRes2 = await client.getSingle({ url: '/ep2', queryParams: { a: 'AAA' } })
      expect(getRes).toEqual({ a: 'b' })
      expect(getRes2).toEqual({ c: 'd' })
    })
    ...
    it('should throw Unauthorized on login 401', async () => {
      ...
      await expect(client.getSingle({ url: '/ep' })).rejects.toThrow(UnauthorizedError)
    })
  })
})
