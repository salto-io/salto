...
describe('RateLimiter', () => {
  it('should handle partial options correctly', () => {
    ...
    const expectedOptions: RateLimiterOptions = {
      ...
      delayMS: RATE_LIMIT_DEFAULT_DELAY_PER_REQUEST_MS,
      startPaused: false,
    }

    expect(rateLimiter.getOptions()).toEqual(expectedOptions)
  })
  ...
  it.each([
    ...
    const rateLimiter = new RateLimiter({ maxConcurrentCalls })
    expect(rateLimiter.getOptions().maxConcurrentCalls).toBe(expected)
  })
  ...
  it.each([
    ...
    expect(rateLimiter.getOptions().delayMS).toBe(expected)
  })
  ...
  it.each([
    ...
    ({ maxCallsPerInterval, intervalLengthMS, expectedCalls, expectedInterval }) => {
      const rateLimiter = new RateLimiter({ maxCallsPerInterval, intervalLengthMS })
      expect(rateLimiter.getOptions().maxCallsPerInterval).toBe(expectedCalls)
      expect(rateLimiter.getOptions().intervalLengthMS).toBe(expectedInterval)
    },
  )
  ...
  it.each([
    ...
    ({ maxCallsPerInterval, intervalLengthMS }) => {
      expect(() => new RateLimiter({ maxCallsPerInterval, intervalLengthMS })).toThrow(
  ...
  it.each([Infinity, 0, -1, undefined])(
    ...
    maxCallsPerInterval => {
      const rateLimiter = new RateLimiter({ maxCallsPerInterval })
      const expectedOptions: RateLimiterOptions = {
        maxConcurrentCalls: Infinity,
        maxCallsPerInterval: Infinity,
        intervalLengthMS: 0,
  ...
  it.each([0, -1, undefined])(
    ...
    intervalLengthMS => {
      const rateLimiter = new RateLimiter({ intervalLengthMS })
      const expectedOptions: RateLimiterOptions = {
        maxConcurrentCalls: Infinity,
        maxCallsPerInterval: Infinity,
        intervalLengthMS: 0,
  ...
  it('should wrap a single async function and execute it', async () => {
    const rateLimiter = new RateLimiter()
  ...
  it('should wrap multiple async functions and execute them', async () => {
    const rateLimiter = new RateLimiter()
