...
import { createServiceIDs, serviceIDKeyCreator, createElemIDFunc, getElemPath } from '../../../src/fetch/element/id_utils'
...
describe('id utils', () => {
  ...
  describe('serviceIDKeyCreator', () => {
    it('should create a stable serialization', () => {
      expect(serviceIDKeyCreator({ serviceIDFields: ['a', 'b'], typeID })({ a: 'A', c: 'C', b: 'B' })).toEqual('{"a":"A","b":"B","object_service_id":"object_name,myAdapter.myType"}')
      expect(serviceIDKeyCreator({ serviceIDFields: ['a', 'b'], typeID })({ b: 'B', a: 'A', c: 'C' })).toEqual('{"a":"A","b":"B","object_service_id":"object_name,myAdapter.myType"}')
      expect(serviceIDKeyCreator({ serviceIDFields: ['b', 'a'], typeID })({ b: 'B', a: 'A', c: 'C' })).toEqual('{"a":"A","b":"B","object_service_id":"object_name,myAdapter.myType"}')
    })
  })

  describe('createElemIDFunc', () => {
    it('should calculate a nacl-cased elem name for a given object and defintion', () => {
      expect(createElemIDFunc({
        elemIDDef: {
          parts: [{ fieldName: 'a' }],
        },
        typeID,
      })({ entry: { a: 'A', b: 'B', c: 'C' }, defaultName: 'unnamed' })).toEqual('A')
    ...
    it('should call getElemIdFunc when service id fields are defined', () => {
      expect(createElemIDFunc({
        ...
        serviceIDDef: ['a'],
      })({ entry: { a: 'A', b: 'B', c: 'C' }, defaultName: 'unnamed' })).toEqual('NAME')
    })
    it('should fallback to default name if constructed name is empty', () => {
      expect(createElemIDFunc({
        elemIDDef: {},
        typeID,
      })({ entry: { a: 'A', b: 'B', c: 'C' }, defaultName: 'unnamed' })).toEqual('unnamed')
      expect(createElemIDFunc({
    ...
    it('should avoid double nacl-case when extending parent', () => {
      ...
      expect(func({ entry: { a: 'A A', b: 'B', c: 'C' }, defaultName: 'unnamed', parent })).toEqual('parent_b__A_A@fuus')
    })
  ...
  describe('getElemPath', () => {
    it('should calculate a nacl-cased elem name for a given object and defintion', () => {
      ...
      })({ entry: { a: 'A', b: 'B', c: 'C' }, defaultName: 'unnamed' })).toEqual(['myAdapter', 'Records', 'myType', 'A'])
    })
    it('should set path to settings folder if singleton', () => {
      ...
      })({ entry: { a: 'A', b: 'B', c: 'C' }, defaultName: 'unnamed' })).toEqual(['myAdapter', 'Records', 'Settings', 'myType'])
    })
  })
})
