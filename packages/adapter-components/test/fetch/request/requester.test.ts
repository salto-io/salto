...
describe('requester', () => {
  ...
  describe('getRequester', () => {
    ...
    it('should make requests and extract items based on definitions', async () => {
      ...
      expect(await requester.request({ typeName: 'myType', contexts: [], requestDef: { endpoint: { path: '/ep' } } })).toEqual([{
        typeName: 'myType',
        context: {},
        value: { a: 'a' },
      }])
      expect(await requester.requestAllForResource({ callerIdentifier: { typeName: 'myType' }, contextPossibleArgs: {} })).toEqual([
        {
          typeName: 'myType',
          context: {},
          value: { a: 'a' },
    ...
    it('should fail if endpoint is not marked as read-only', async () => {
      ...
      await expect(() => requester.request({ typeName: 'myType', contexts: [], requestDef: { endpoint: { path: '/ep' } } })).rejects.toThrow('Endpoint [main]/ep:undefined is not marked as readonly, cannot use in fetch')
      await expect(() => requester.requestAllForResource({ callerIdentifier: { typeName: 'myType' }, contextPossibleArgs: {} })).rejects.toThrow('Endpoint [main]/ep:undefined is not marked as readonly, cannot use in fetch')
    })
  })
})
