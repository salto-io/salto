...
*/
import { findAllUnresolvedArgs, findUnresolvedArgs, replaceAllArgs, replaceArgs } from '../../../src/fetch/request/utils'
...
describe('request utils', () => {
  ...
  describe('replaceAllArgs', () => {
    it('should replace found args and keep the rest', () => {
      expect(replaceAllArgs({ value: { x: { y: 'a{b}c{d123} {e} {' } }, context: { e: 'E' } })).toEqual({ x: { y: 'a{b}c{d123} E {' } })
      expect(replaceAllArgs({ value: { x: { y: ['a{b}c{d123} {e} {'] } }, context: { e: 'E' } })).toEqual({ x: { y: ['a{b}c{d123} E {'] } })
    })
  })
})
