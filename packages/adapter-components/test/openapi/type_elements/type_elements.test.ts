...
import { swaggerTypeToPrimitiveType } from '../../../src/openapi/type_elements/swagger_parser'
import { generateTypes } from '../../../src/openapi'
import { RequestableTypeSwaggerConfig } from '../../../src/config_deprecated'
...
const BASE_DIR = __dirname.replace('openapi/type_elements', 'fetch/element/openAPI')
...
describe('swagger_type_elements', () => {
  describe('generateTypes', () => {
    ...
    describe('no config overrides', () => {
      const validateV2 = async (
        ...
        const { allTypes, parsedConfigs } = await generateTypes(ADAPTER_NAME, {
          swagger: { url },
          typeDefaults: {
            transformation: {
              idFields: ['name'],
      ...
      const validateV3 = async (url: string): ReturnType<typeof generateTypes> => {
    ...
    describe('with config overrides', () => {
      ...
      beforeAll(async () => {
        const res = await generateTypes(ADAPTER_NAME, {
          swagger: {
          ...
          types: {
            Order: {
              transformation: {
                ...
                fieldsToHide: [{ fieldName: 'petId' }, { fieldName: 'newHiddenField' }],
              },
            },
            NewType: {
              transformation: {
      ...
      it('should override field types', async () => {
        ...
        expect(await ((await order.fields.petId.getType()) as ListType).getInnerType()).toEqual(BuiltinTypes.NUMBER)
        ...
        expect(await ((await order.fields.quantity.getType()) as MapType).getInnerType()).toEqual(allTypes.Category)
      })
      it('should add fields that did not already exist', async () => {
        ...
        expect(await ((await order.fields.additionalTypes.getType()) as ListType).getInnerType()).toEqual(
          additionalType,
        )
      })
  ...
  describe('swaggerTypeToPrimitiveType', () => {
    it('should return the right primitive type when one is specified', () => {
      expect(swaggerTypeToPrimitiveType('string')).toEqual(BuiltinTypes.STRING)
      expect(swaggerTypeToPrimitiveType('byte')).toEqual(BuiltinTypes.STRING)
      expect(swaggerTypeToPrimitiveType('number')).toEqual(BuiltinTypes.NUMBER)
      expect(swaggerTypeToPrimitiveType('date')).toEqual(BuiltinTypes.STRING)
    })
    it('should return unknown when type is not known', () => {
      expect(swaggerTypeToPrimitiveType('bla')).toEqual(BuiltinTypes.UNKNOWN)
    })
    it('should return correct type when multiple types that map to the same builtin are specified', () => {
      expect(swaggerTypeToPrimitiveType(['string', 'byte'])).toEqual(BuiltinTypes.STRING)
    })
    it('should return unknown when multiple conflicting types are specified', () => {
      expect(swaggerTypeToPrimitiveType(['string', 'number'])).toEqual(BuiltinTypes.UNKNOWN)
    })
  })
})
