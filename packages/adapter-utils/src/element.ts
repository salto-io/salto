...
type SaltoAnnotationsForField<T, K> =
  K extends OptionalKeys<T>
    ? { annotations?: { _required?: false } }
    : { annotations: { _required: true } }

type ObjectTypeCtorForType<T> = (
  Omit<ConstructorParameters<typeof ObjectType>[0], 'fields'>
  & {
    fields: {
 ...
 */
export const createMatchingObjectType = <T>(params: ObjectTypeCtorForType<T>): ObjectType => (
  new ObjectType(params)
)
