  ...
  describe('extractAdditionalPropertiesField', () => {
    ...
    describe('when there is no additional properties annotation', () => {
      it('should return Field with unknown type', () => {
        expect(
          extractAdditionalPropertiesField(baseObj, fieldName)
        ).toEqual(new Field(baseObj, fieldName, BuiltinTypes.UNKNOWN))
      })
    })
    describe('when additional properties annotation is false', () => {
      it('should return undefined', () => {
        ...
        expect(
          extractAdditionalPropertiesField(objType, fieldName)
        ).toBeUndefined()
      })
    })
    describe('when additional properties annotation is defining a type', () => {
      it('should return undefined', () => {
        ...
        expect(
          extractAdditionalPropertiesField(objType, fieldName)
        ).toEqual(new Field(objType, fieldName, additionalPropertiesObjType, { someUniqueAnnotation: 'unique' }))
      })
    })
  })
})
