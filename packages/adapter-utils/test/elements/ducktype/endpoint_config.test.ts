...
describe('ducktype_endpoint_config', () => {
  describe('createAdapterApiConfigType', () => {
    it('should return default type when no custom fields were added', async () => {
      ...
      const endpoints = await type.fields.endpoints.getType() as MapType
      expect(endpoints).toBeInstanceOf(MapType)
      const endpointsInner = await endpoints.getInnerType() as ObjectType
      ...
      const endpoint = await endpointsInner.fields.endpoint.getType() as ObjectType
      const translation = await endpointsInner.fields.translation.getType() as ObjectType
