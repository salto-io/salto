...
import { blueprints as sbp, state, csv } from 'salto'
...
describe('When running export', () => {
  ...
  beforeEach(async () => {
    ...
    await asyncfile.delete(state.STATEPATH)
  })
  ...
  it('should save the data in csv file after discover', async () => {
    ...
    await exportCommand(await sbp.loadBlueprints([], discoverOutputDir), sfLeadObjectName,
  ...
  it('should fail if discover was not run beforehand', async () => {
    const command = exportCommand(
      await sbp.loadBlueprints([], discoverOutputDir),
      sfLeadObjectName,
...
describe('When running data modifying commands', () => {
  ...
  describe('When running import from a CSV file', () => {
    ...
    it('should succeed after discover', async () => {
      ...
      await discover(discoverOutputDir, []).execute()
      await importCommand(await sbp.loadBlueprints([], discoverOutputDir), dataFilePath,
    ...
    it('should fail if discover was not run beforehand', async () => {
      const cliOutput = { stdout: new MockWriteStream(), stderr: new MockWriteStream() }
      const command = importCommand(await sbp.loadBlueprints([], discoverOutputDir), dataFilePath,
  ...
  describe('When running delete instances read from a CSV file', () => {
    ...
    it('should succeed after discover', async () => {
      ...
      await exportCommand(await sbp.loadBlueprints([], discoverOutputDir), sfLeadObjectName,
        exportOutputFullPath).execute()
      const exportObjects = await csv.readCsv(exportOutputFullPath)
      const clark = exportObjects.find(o => o.FirstName === 'Clark' && o.LastName === 'Kent')
      const bruce = exportObjects.find(o => o.FirstName === 'Bruce' && o.LastName === 'Wayne')

      const deletionObjects = await csv.readCsv(dataFilePath)
      deletionObjects[0].Id = clark.Id
      deletionObjects[1].Id = bruce.Id

      await csv.dumpCsv(deletionObjects, updatedDataFilePath, false)

      await deleteCommand(await sbp.loadBlueprints([], discoverOutputDir), updatedDataFilePath,
        sfLeadObjectName, cliOutput).execute()
    ...
    it('should fail if discover was not run beforehand', async () => {
      const cliOutput = { stdout: new MockWriteStream(), stderr: new MockWriteStream() }
      const command = deleteCommand(await sbp.loadBlueprints([], discoverOutputDir), dataFilePath,
        sfLeadObjectName, cliOutput)
      await expect(command.execute()).rejects
        .toThrow(`Couldn't find the type you are looking for: ${sfLeadObjectName}. Have you run salto discover yet?`)
    })
