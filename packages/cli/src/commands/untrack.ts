...
type UntrackParsedCliInput = ParsedCliInput<UntrackArgs>
...
export const command = (
  ...
  async execute(): Promise<CliExitCode> {
    log.debug(`running untrack command on '${workspaceDir}' [environment=${inputEnvironment}, inputSelectors=${inputSelectors}`)
...
const untrackeBuilder = createCommandBuilder({
  options: {
    command: 'untrack [selectors..]',
    description: 'Move the selected elements from the common folder - unsharing them between all environments',
    keyed: {
      force: {
        alias: ['f'],
  ...
  async build(
    input: UntrackParsedCliInput,
    output: CliOutput,
    spinnerCreator: SpinnerCreator
  ): Promise<CliCommand> {
    return command(
      '.',
      getCliTelemetry(input.telemetry, 'untrack'),
      output,
      spinnerCreator,
      input.args.force,
...
export default untrackeBuilder
