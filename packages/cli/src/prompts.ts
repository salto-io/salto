...
export const deployOrValidate = (checkOnly: boolean, capitalize: boolean, suffix: boolean): string => {
  if (checkOnly) {
    const validate = suffix ? 'validation' : 'validate'
    return capitalize ? _.capitalize(validate) : validate
  }
  const deploy = suffix ? 'deployment' : 'deploy'
...
export default class Prompts {
  ...
  public static readonly SHOULD_EXECUTE_DEPLOY_PLAN = (checkOnly: boolean): string => `Do you want to ${deployOrValidate(checkOnly, false, false)}?`

  public static readonly CANCEL_DEPLOY_ACTION = 'Cancelled: Due to an erroneous dependency -'
  public static readonly START_DEPLOY_EXEC = (checkOnly: boolean): string => `Starting the ${deployOrValidate(checkOnly, false, true)} plan`
  ...
  public static readonly CHANGES_DEPLOY_SUMMARY = (numChanges: number, checkOnly: boolean): string => `${deployOrValidate(checkOnly, true, true)} succeeded - ${numChanges} applied change(s).`
  public static readonly ERRORS_DEPLOY_SUMMARY = (numErrors: number, checkOnly: boolean): string => `${deployOrValidate(checkOnly, true, true)} failed with ${numErrors} error(s).`
  public static readonly CANCEL_DEPLOY = (checkOnly: boolean): string => `Cancelling ${deployOrValidate(checkOnly, false, true)}`
