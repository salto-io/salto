...
const detailedNestedElementErrorMessage = (path: ElemID, action: Change['action']): string => {
  const nestedPart = path
    .getFullNameParts()
    .slice(path.nestingLevel * -1)
    .join('.')
...
const getUnsupportedPaths = async (change: Change<InstanceElement>): Promise<{ field: ElemID; path: ElemID }[]> => {
  ...
  await transformValues({
    ...
    transformFunc: async ({ value, field, path }) => {
      if (field !== undefined && !isDeploymentSupported(field, change.action) && path !== undefined) {
