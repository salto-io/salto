...
describe('summarizeDeployChanges', () => {
  const [,, saltoOffice,
    saltoEmployee,
    saltoEmployeeInstance,,,, anotherSaltoEmployeeInstance,
  ...
  const createEmployee = (value: Values): InstanceElement => new InstanceElement(
    'instance',
    saltoEmployee,
    value
  )
  const firstEmployeeInstance = createEmployee(
    { name: 'FirstEmployee', nicknames: ['you', 'hi'] }
  )
  ...
  describe('addition changes', () => {
    it('should return the requested change ids as success, when added correctly', () => {
      const modifyRequestedChanges: Change[] = [{ action: 'add',
        data: { after: firstEmployeeInstance } }]
      const appliedChanges: Change[] = [{ action: 'add',
        data: { after: firstEmployeeInstance } }]
    ...
    it('should return the requested change ids as partial-success, when they were partially added', () => {
      const modifyRequestedChanges: Change[] = [{ action: 'add',
        data: { after: firstEmployeeInstance } }]
      const appliedChanges: Change[] = [{ action: 'add',
        data: { after: createEmployee({ name: 'FirstEmployee' }) } }]
      expect(summarizeDeployChanges(modifyRequestedChanges, appliedChanges)).toEqual({
    ...
    it('should return the requested change ids as failure, when applied as modification', () => {
      const modifyRequestedChanges: Change[] = [{ action: 'add',
        data: { after: firstEmployeeInstance } }]
      const appliedChanges: Change[] = [{ action: 'modify',
        data: { before: firstEmployeeInstance, after: firstEmployeeWithNewNickNameAndOffice } }]
      expect(summarizeDeployChanges(modifyRequestedChanges, appliedChanges)).toEqual({
    ...
    it('should return the requested change ids as failure, when they wrongfully got removed', () => {
      const modifyRequestedChanges: Change[] = [{ action: 'add',
        data: { after: firstEmployeeInstance } }]
      const appliedChanges: Change[] = [{ action: 'remove',
        data: { before: firstEmployeeInstance } }]
      expect(summarizeDeployChanges(modifyRequestedChanges, appliedChanges)).toEqual({
        [firstEmployeeInstance.elemID.getFullName()]: 'failure',
      })
    })
  ...
  describe('removal changes', () => {
    it('should return the requested change ids as failure, when they wrongfully got added', () => {
      const modifyRequestedChanges: Change[] = [{ action: 'remove',
        data: { before: firstEmployeeInstance } }]
      const appliedChanges: Change[] = [{ action: 'add',
        data: { after: firstEmployeeInstance } }]
      expect(summarizeDeployChanges(modifyRequestedChanges, appliedChanges)).toEqual(
  ...
  describe('modify changes', () => {
    ...
    it('should return the requested change ids as failure, when they wrongfully got added', () => {
      const modifyRequestedChanges: Change[] = [{ action: 'modify',
        data: { before: firstEmployeeInstance, after: firstEmployeeWithNewNickNameAndOffice } }]
      const appliedChanges: Change[] = [{ action: 'add',
        data: { after: firstEmployeeWithNewNickNameAndOffice } }]
      expect(summarizeDeployChanges(modifyRequestedChanges, appliedChanges)).toEqual({
        [saltoEmployeeInstanceInstanceID.createNestedID('nicknames', '1')
          .getFullName()]: 'failure',
    ...
    it('should return the requested modified detailed changes ids as failure, when they were not applied', () => {
      ...
      const appliedRequestedChanges: Change[] = [{ action: 'modify',
        data: {
          before: firstEmployeeInstance,
    ...
    it('should return the requested detailed changes ids as partial success when they were partially created', () => {
      ...
      const appliedChanges: Change[] = [{ action: 'modify',
        data: { before: firstEmployeeInstance,
          after: firstEmployeeWithNewNickNameAndOfficePartiallyCreated } }]
      expect(summarizeDeployChanges(modifyRequestedChanges, appliedChanges)).toEqual({
        [saltoEmployeeInstanceInstanceID.createNestedID('nicknames', '1')
          .getFullName()]: 'success',
        [saltoEmployeeInstanceInstanceID.createNestedID('office')
          .getFullName()]: 'partial-success',
      })
    })
  ...
  describe('changes with different actions', () => {
    it('should return all changes as failure then they were not applied', () => {
      ...
      expect(summarizeDeployChanges(requestedChanges, [])).toEqual({
        ...
        [saltoEmployeeInstanceInstanceID.createNestedID('hobby')
          .getFullName()]: 'failure',
      })
    })
    it('should return requested change ids as failure when none were applied', () => {
      expect(summarizeDeployChanges(AddRemoveChanges, []))
        .toEqual(
    ...
    it('should return failures on changes that were not applied', () => {
      expect(summarizeDeployChanges(AddRemoveChanges, [AddRemoveChanges[0]]))
        .toEqual(
    ...
    it('should return success summary when all changes were applied', () => {
      expect(summarizeDeployChanges(AddRemoveChanges, AddRemoveChanges))
        .toEqual(
          { [saltoEmployee.elemID.getFullName()]: 'success',
