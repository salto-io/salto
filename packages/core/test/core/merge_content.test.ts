...
describe('merge contents', () => {
  ...
  describe('merge strings', () => {
    it('should merge modified value', () => {
      expect(mergeStrings(changeId, { current, base, incoming: incomingMergeable }))
        .toEqual(modifiedMerged)
    })
    it('should merge added value', () => {
      expect(mergeStrings(changeId, { current, base: undefined, incoming: incomingAdded }))
        .toEqual(addedMerged)
    })
    it('should not merge unmergeable modified value', () => {
      expect(mergeStrings(changeId, { current, base, incoming: incomingUnmergeable }))
        .toBeUndefined()
    })
    it('should not merge unmergeable added value', () => {
      expect(mergeStrings(changeId, { current, base: undefined, incoming: incomingUnmergeable }))
        .toBeUndefined()
    })
    it('should not merge too long value', () => {
      const incomingTooLong = 'hello world!\nmy name is:\nNaCls the great!'.padEnd(10 * 1024 * 1024 + 1, '!')
      expect(mergeStrings(changeId, { current, base, incoming: incomingTooLong }))
        .toBeUndefined()
    })
    ...
    it('should not merge on unknown error', () => {
      jest.spyOn(diff3, 'mergeDiff3').mockImplementationOnce(() => { throw new Error() })
      jest.spyOn(diff3, 'diffComm').mockImplementationOnce(() => { throw new Error() })
      expect(mergeStrings(changeId, { current, base, incoming: incomingMergeable }))
        .toBeUndefined()
      expect(mergeStrings(changeId, { current, base: undefined, incoming: incomingAdded }))
        .toBeUndefined()
  ...
  describe('merge static files', () => {
    ...
    it('should merge modified file', async () => {
      expect(await mergeStaticFiles(
        changeId,
        { current: currentFile, base: baseFile, incoming: incomingMergeableFile },
      )).toEqual(modifiedMergedFile)
    })
    it('should merge added file', async () => {
      expect(await mergeStaticFiles(
        changeId,
        { current: currentFile, base: undefined, incoming: incomingAddedFile },
      )).toEqual(addedMergedFile)
    })
    it('should not merge unmergeable modified file', async () => {
      expect(await mergeStaticFiles(
        changeId,
        { current: currentFile, base: baseFile, incoming: incomingUnmergeableFile },
      )).toBeUndefined()
    })
    it('should not merge unmergeable added file', async () => {
      expect(await mergeStaticFiles(
        changeId,
        { current: currentFile, base: undefined, incoming: incomingUnmergeableFile },
      )).toBeUndefined()
    })
    it('should not merge if filepath not match', async () => {
      ...
      expect(await mergeStaticFiles(
        changeId,
        { current: currentFile, base: baseFile, incoming: mergeableFileOtherPath },
      )).toBeUndefined()
    })
    it('should not merge if filepath is binary', async () => {
      ...
      expect(await mergeStaticFiles(
        changeId,
        { current: currentBinaryPathFile, base: baseBinaryPathFile, incoming: incomingBinaryPathFile },
      )).toBeUndefined()
    })
    it('should not merge if content is missing', async () => {
      ...
      expect(await mergeStaticFiles(
        changeId,
        { current: currentFile, base: baseFile, incoming: incomingNoContent },
      )).toBeUndefined()
    })
    it('should not merge if content is binary', async () => {
      ...
      expect(await mergeStaticFiles(
        changeId,
        { current: currentFile, base: baseFile, incoming: binaryFileContent },
      )).toBeUndefined()
    })
  })
})
