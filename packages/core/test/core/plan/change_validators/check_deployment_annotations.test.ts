...
describe('checkDeploymentAnnotationsValidator', () => {
  ...
  it('should not return an error when the changed element is not an instance', async () => {
    const errors = await checkDeploymentAnnotationsValidator([toChange({ after: type })])
    expect(errors).toEqual([])
  })

  it('should return an error when type does not support operation', async () => {
    const errors = await checkDeploymentAnnotationsValidator([toChange({ after: instance })])
    expect(errors).toEqual([
      {
  ...
  it('should return an error when a field does not support a modification operation', async () => {
    ...
    const errors = await checkDeploymentAnnotationsValidator([toChange({ before: instance, after: afterInstance })])
    expect(errors).toEqual([
      {
        ...
        detailedMessage:
          'Deploying "notUpdatableField" in adapter.test.instance.instance is not supported. The current value in the target environment will not be modified',
      },
    ])
  })
  ...
  it('should return an error when a field does not support an addition operation', async () => {
    ...
    const errors = await checkDeploymentAnnotationsValidator([toChange({ after: afterInstance })])
    expect(errors).toEqual([
      {
        elemID: instance.elemID.createNestedID('notUpdatableField'),
        severity: 'Info',
        message: 'Operation not supported for specific value',
        detailedMessage:
          'Deploying "notUpdatableField" in adapter.test.instance.instance is not supported. The instance will be created with the default value of the target environment',
      },
    ])
  })
