...
*/
import { InstanceElement, DeployResult, ChangeDataType, getChangeData } from '@salto-io/adapter-api'
...
describe('Hubspot Adapter Operations', () => {
  ...
  describe('Add operation', () => {
    ...
    describe('When instance name already exists', () => {
      ...
      it('should return error (409 response)', async () => {
        const result = await adapterAdd(formInstance)
        expect(result.errors).toHaveLength(1)
        expect(result.errors[0].message).toMatch(formAlreadyExistErrStr)
      })
    })
    ...
    describe('Wrong apikey', () => {
      ...
      it('should return error (401 response)', async () => {
        const result = await adapterAdd(formInstance)
        expect(result.errors).toHaveLength(1)
        expect(result.errors[0].message).toMatch(apikeyDoesntExistErrStr)
      })
    })
    ...
    describe('When an instance is successfully added', () => {
      describe('Form instance', () => {
        ...
        it('should return the new form', async () => {
          const deployResult = await adapterAdd(formInstance)
          expect(deployResult.errors).toHaveLength(0)
          expect(deployResult.appliedChanges).toHaveLength(1)
          const result = getChangeData(deployResult.appliedChanges[0]) as InstanceElement
      ...
      describe('ContactProperty instance', () => {
        ...
        it('should return the new ContactProperty', async () => {
          const deployResult = await adapterAdd(contactPropertyInstance)
          expect(deployResult.errors).toHaveLength(0)
          expect(deployResult.appliedChanges).toHaveLength(1)
          const res = getChangeData(deployResult.appliedChanges[0]) as InstanceElement
      ...
      describe('Workflow instance', () => {
        ...
        it('should return the new Workflow', async () => {
          const deployResult = await adapterAdd(workflowsInstance)
          expect(deployResult.errors).toHaveLength(0)
          expect(deployResult.appliedChanges).toHaveLength(1)
          const result = getChangeData(deployResult.appliedChanges[0]) as InstanceElement
      ...
      describe('MarketingEmail instance', () => {
        ...
        it('should return the new MarketingEmail', async () => {
          const deployResult = await adapterAdd(marketingEmailInstance)
          expect(deployResult.errors).toHaveLength(0)
          expect(deployResult.appliedChanges).toHaveLength(1)
          const result = getChangeData(deployResult.appliedChanges[0]) as InstanceElement
    ...
    describe('When trying create form with wrong values', () => {
      ...
      it('should return error (403 response)', async () => {
        const result = await adapterAdd(formInstance)
        expect(result.errors).toHaveLength(1)
        expect(result.errors[0].message).toMatch(privilegeErrStr)
      })
    })
  ...
  describe('Remove operation', () => {
    ...
    describe('When remove fails', () => {
      ...
      it('should return error (401 response)', async () => {
        const result = await adapterRemove(formInstance)
        expect(result.errors).toHaveLength(1)
        expect(result.errors[0].message).toMatch(apikeyDoesntExistErrStr)
      })
    })
    ...
    describe('When remove success', () => {
      ...
      it('should return 204 response (Form type)', async () => {
        ...
        expect(getChangeData(result.appliedChanges[0]).elemID).toEqual(formInstance.elemID)
      })

      it('should return 204 response (Workflow type)', async () => {
        const result = await adapterRemove(workflowsInstance)
        expect(result.errors).toHaveLength(0)
        expect(result.appliedChanges).toHaveLength(1)
        expect(getChangeData(result.appliedChanges[0]).elemID).toEqual(workflowsInstance.elemID)
      })

      it('should return 204 response (MarketingEmail type)', async () => {
        const result = await adapterRemove(marketingEmailInstance)
        expect(result.errors).toHaveLength(0)
        expect(result.appliedChanges).toHaveLength(1)
        expect(getChangeData(result.appliedChanges[0]).elemID).toEqual(
          marketingEmailInstance.elemID
        )
      })

      it('should return 204 response (ContactProperty type', async () => {
        const result = await adapterRemove(contactPropertyInstance)
        expect(result.errors).toHaveLength(0)
        expect(result.appliedChanges).toHaveLength(1)
        expect(getChangeData(result.appliedChanges[0]).elemID).toEqual(
          contactPropertyInstance.elemID
        )
      })
    })
  })
  ...
  describe('Update operation', () => {
    ...
    describe('When Update success', () => {
      describe('Form type', () => {
        ...
        it('should return the updated form', async () => {
          const deployResult = await adapterUpdate(beforeUpdateInstance, afterUpdateInstance)
          expect(deployResult.errors).toHaveLength(0)
          expect(deployResult.appliedChanges).toHaveLength(1)
          const res = getChangeData(deployResult.appliedChanges[0]) as InstanceElement
    ...
    describe('When Instance was not found', () => {
      ...
      it('should return 404 response', async () => {
        ...
        expect(deployResult.errors[0].message).toMatch(noFormFoundErrStr)
      })
    })
    ...
    describe('When Forms have different guids', () => {
      ...
      it('should return error', async () => {
        const deployResult = await adapterUpdate(beforeUpdateInstance, afterUpdateInstance)
        expect(deployResult.errors).toHaveLength(1)
        expect(deployResult.errors[0].message).toMatch("Failed to update element as guid's prev=")
      })
    })
  })
})
