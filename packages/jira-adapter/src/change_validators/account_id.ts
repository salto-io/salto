...
const noDisplayNameChangeError = (elemId: ElemID): ChangeError => ({
  elemID: elemId,
  severity: 'Info',
...
const doesDefaultUserExist = (defaultUser: string | undefined, userMap: UserMap, isDataCenter: boolean): boolean => {
...
const createChangeErrorsForAccountIdIssues = (
  ...
  walkOnElement({
    element,
    func: walkOnUsers(
      checkAndAddChangeErrors(userMap, isDataCenter, allMissingUsers, displayNameMismatches, missingDisplayNamePaths),
      config,
    ),
  })
  ...
  if (missingUsers.length > 0) {
    if (config.deploy.defaultMissingUserFallback === undefined) {
      ...
      changeErrors.push(
        noFallbackUserAccountIdChangeError({
          elemId: element.elemID,
          missingUsers,
          defaultUser: config.deploy.defaultMissingUserFallback,
        }),
  ...
  displayNameMismatches.forEach(mismatch => {
    changeErrors.push(
      displayNameMismatchChangeError({
 ...
 */
export const accountIdValidator: (client: JiraClient, config: JiraConfig) => ChangeValidator =
