...
export const customFieldsWith10KOptionValidator: ChangeValidator = async changes => changes
  ...
  .map(instance => ({
    ...
    severity: 'Error',
    message: 'Cannot deploy custom field with more than 10K options',
    detailedMessage: `Cannot deploy custom field ${getParents(instance)[0].elemID.name} because it's associated context ${instance.elemID.name} has more than 10K options.`,
  }))
