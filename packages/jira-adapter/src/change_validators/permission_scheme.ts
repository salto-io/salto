...
const isPermissionSchemeAssociationChange = (
  ...
  const elemBefore =
    isModificationChange(change) && isResolvedReferenceExpression(change.data.before.value.permissionScheme)
...
export const permissionSchemeDeploymentValidator =
  ...
  async (changes, elementsSource) => {
    if (client.isDataCenter || elementsSource === undefined || !(await isAllFreeLicense(elementsSource)) || !changes) {
      return []
    }
    ...
    const associatedSchemes = new Set(
    ...
    const associatedSchemeChangeErrors = projectAndSchemeChanges
      ...
      .map(change =>
        isAdditionChange(change) && schemesThatWereAddedWithProjects.has(getChangeData(change).elemID.getFullName())
    ...
    const unassociatedSchemeChangeErrors = projectAndSchemeChanges
      .filter(
        change =>
          isAdditionChange(change) || schemesThatWereAssociatedBefore.has(getChangeData(change).elemID.getFullName()),
      )
      .map(getChangeData)
