...
export const statusMigrationChangeValidator: ChangeValidator = async changes => {
  ...
  const invalidItemErrors = _.remove(relevantChanges, statusMigrationHasInvalidItem).map(
    generateStatusMigrationInvalidError,
  )
  const repeatingItemErrors = relevantChanges
    .map(change => {
