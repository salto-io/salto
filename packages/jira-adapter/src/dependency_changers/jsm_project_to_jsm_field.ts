...
import { AdditionChange, DependencyChange, DependencyChanger, InstanceElement, dependencyChange, getChangeData, isAdditionChange, isInstanceChange } from '@salto-io/adapter-api'
...
export const jsmProjectToJsmFieldDependencyChanger: DependencyChanger = async changes => {
  const instanceChanges = Array.from(changes.entries())
    ...
    .filter(
      (change): change is deployment.dependency.ChangeWithKey<AdditionChange<InstanceElement>> =>
  ...
  const [projectChanges, fieldChanges] = _.partition(
    relevantChanges,
    change =>
      getChangeData(change.change).elemID.typeName === PROJECT_TYPE
  )
  ...
  return jsmProjects.flatMap(change => {
    ...
    return jsmFields.map(
      fieldChange => createDependencyChange(jsmProjectChange, fieldChange)
    ).flat()
  })
}
