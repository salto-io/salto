...
import { deployment, client as clientUtils, resolveValues } from '@salto-io/adapter-components'
...
const deployChange = async (
  ...
  const instance = await deployment.filterIgnoredValues(
    await deployment.filterUndeployableValues(await resolveValues(getChangeData(change), getLookUpName), change.action),
    fieldsToIgnore,
  )
...
const queryServiceValues = async (
  ...
  const queryFromClient = async (url: string): Promise<clientUtils.ResponseValue[]> => {
    ...
    const data =
      urls.dataField !== undefined && !Array.isArray(response.data) ? response.data[urls.dataField] : response.data
  ...
  const data = queryFunction !== undefined ? await queryFunction() : await queryFromClient(urls.query as string)

  const serviceValues =
    getNameFunction !== undefined
      ? data.map(values => ({
          ...values,
...
const addIdsToResults = async (
  ...
  const nameToId = Object.fromEntries(serviceValues.map(value => [value.name, value.id]))
...
const verifyDeployment = async (
  ...
  const invalidChanges = _.remove(deployResult.appliedChanges, change => {
    ...
    const serviceValue = serviceValuesTransformer(idToValues[instance.value.id] as Values, instance)

    if (!_.isMatch(serviceValue, instance.value)) {
      log.warn(
        `Failed to deploy ${instance.elemID.getFullName()}, the deployment values  ${safeJsonStringify(instance.value)} do not match the service values ${safeJsonStringify(serviceValue)}`,
      )
      return true
    }
  ...
  const message = 'Failed to deploy change'
  deployResult.errors = [
    ...deployResult.errors,
...
export const deployWithJspEndpoints = async ({
  ...
  const deployResult = await deployChanges(changes, change => deployChange(change, client, urls, fieldsToIgnore))
