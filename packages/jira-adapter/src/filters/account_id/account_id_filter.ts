...
const addToCache = (cache: AccountIdsCache, path: ElemID, objectToClone: Value): void => {
...
const walkOnAutomationValue =
  (regexPath: string, callback: WalkOnUsersCallback, excludeValues?: string[]): WalkOnFunc =>
  ({ value, path }): WALK_NEXT_STEP => {
    if (
      new RegExp(regexPath).test(path.getFullName()) &&
      value.type === 'ID' &&
...
const walkOnUserCondition =
  (callback: WalkOnUsersCallback): WalkOnFunc =>
  ({ value, path }): WALK_NEXT_STEP => {
    if (value !== undefined && typeof value.check === 'string' && value.check.startsWith('USER_')) {
      walkOnValue({
        value: value.criteria,
        elemId: path.createNestedID('criteria'),
        func: walkOnAutomationValue('value\\.conditions\\.\\d+\\.criteria\\.\\d+', callback),
      })
      return WALK_NEXT_STEP.SKIP
    }
    return WALK_NEXT_STEP.RECURSE
...
const accountIdsScenarios = (
  ...
  if (
    PARAMETER_STYLE_TYPES.includes(path.typeName) &&
  ...
  if (OWNER_STYLE_TYPES.includes(path.typeName) && value.owner !== undefined) {
  ...
  if (path.typeName === BOARD_TYPE_NAME && path.name === 'users' && Array.isArray(value)) {
    _.range(value.length).forEach(index =>
      callback({
        value,
        path,
  ...
  if (path.typeName === AUTOMATION_TYPE) {
    ...
    if (
      USER_TYPE_FIELDS.includes(value.selectedFieldType) &&
    ...
    if (USER_TYPE_FIELDS.includes(value.fieldType)) {
      walkOnValue({
        ...
        func: walkOnAutomationValue(
          'value\\.operations\\.\\d+.value\\.\\d+|value\\.operations\\.\\d+\\.value',
          callback,
          ['assignee', 'reporter'],
        ),
      })
    ...
    if (value.type === 'jira.user.condition') {
      walkOnValue({
        value: value.value?.conditions,
        elemId: path.createNestedID('value', 'conditions'),
        func: walkOnUserCondition(callback),
      })
    }
    // assign action
...
export const walkOnUsers =
  (callback: WalkOnUsersCallback, config: JiraConfig): WalkOnFunc =>
  ({ value, path }): WALK_NEXT_STEP => {
    if (isInstanceElement(value)) {
      return isAccountIdType(value) ? accountIdsScenarios(value.value, path, callback, config) : WALK_NEXT_STEP.EXIT
    }
    if (value != null) {
      // null or undefined
...
const cacheAndSimplifyAccountId =
  (cache: AccountIdsCache): WalkOnUsersCallback =>
  ({ value, path, fieldName }): void => {
...
const convertType = async (objectType: ObjectType): Promise<void> => {
  ...
  if (isListType(await objectType.fields.accountIds?.getType())) {
    objectType.fields.accountIds.refType = new TypeReference(accountIdInfoListType.elemID, accountIdInfoListType)
  }
}
/*
  ...
  return {
    ...
    onFetch: async elements => {
      ...
      await awu(elements)
        ...
        .filter(
          object =>
            ACCOUNT_ID_TYPES.includes(object.elemID.typeName) ||
