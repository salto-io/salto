...
const isTimeTrackingResponse = createSchemeGuard<TimeTrackingResponse>(
  TIME_TRACKING_RESPONSE_SCHEME,
  'Received invalid time tracking result',
)

const getTimeTracking = async (instance: InstanceElement, client: JiraClient): Promise<string | undefined> => {
...
export const deployEstimation = async (
  ...
  if (
    (isModificationChange(resolvedChange) &&
      _.isEqual(resolvedChange.data.before.value.estimation, resolvedChange.data.after.value.estimation)) ||
  ...
  await client.putPrivate({
    ...
    data: {
      ...
      trackingStatisticId:
        instance.value.estimation.timeTracking !== undefined
...
const filter: FilterCreator = ({ config, client }) => ({
  ...
  onFetch: async (elements: Element[]) => {
    ...
    await Promise.all(
      boardInstances.map(async instance => {
