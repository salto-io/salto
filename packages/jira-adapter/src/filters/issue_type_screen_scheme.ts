...
const deployDefaultMapping = async (change: ModificationChange<InstanceElement>, client: JiraClient): Promise<void> => {
  const defaultBefore = change.data.before.value.issueTypeMappings?.find(
    (mapping: Values) => mapping.issueTypeId === DEFAULT_ISSUE_TYPE,
  )
  const defaultAfter = change.data.after.value.issueTypeMappings?.find(
    (mapping: Values) => mapping.issueTypeId === DEFAULT_ISSUE_TYPE,
  )
...
const filter: FilterCreator = ({ config, client }) => ({
  ...
  onFetch: async elements => {
    ...
    const issueTypeScreenSchemaType = types.find(element => element.elemID.name === ISSUE_TYPE_SCREEN_SCHEME_NAME)

    if (issueTypeScreenSchemaType === undefined) {
      log.warn(`${ISSUE_TYPE_SCREEN_SCHEME_NAME} type was not found`)
    } else {
      issueTypeScreenSchemaType.fields.issueTypeMappings.annotations[CORE_ANNOTATIONS.UPDATABLE] = true
    }

    const issueTypeScreenSchemaItemType = types.find(
      element => element.elemID.name === ISSUE_TYPE_SCREEN_SCHEME_ITEM_NAME,
    )

    if (issueTypeScreenSchemaItemType === undefined) {
      log.warn(`${ISSUE_TYPE_SCREEN_SCHEME_ITEM_NAME} type was not found`)
    } else {
      issueTypeScreenSchemaItemType.fields.issueTypeId.annotations[CORE_ANNOTATIONS.UPDATABLE] = true

      issueTypeScreenSchemaItemType.fields.screenSchemeId.annotations[CORE_ANNOTATIONS.UPDATABLE] = true
    }
  },
  deploy: async changes => {
    ...
    const deployResult = await deployChanges(
      ...
      async change => deployIssueTypeScreenSchema(await resolveChangeElement(change, getLookUpName), client, config),
    )
