...
import { referenceFunc, walkOnScripts } from './workflow_script_references'
...
const removeTemplateReferences = (originalInstances: Record<string, TemplateExpression>): referenceFunc => (
  ...
  replaceTemplatesWithValues(
    ...
    (part: ReferenceExpression): TemplatePart => {
      ...
      throw new Error(`Received an invalid value inside a template expression of ScriptRunner ${part.elemID.getFullName()}`)
    }
  )
}
...
const filter: FilterCreator = ({ config, client }) => {
  ...
  return {
    ...
    preDeploy: async changes => {
      ...
      walkOnScripts({ func: removeTemplateReferences(deployTemplateMapping),
    ...
    onDeploy: async changes => {
      ...
      walkOnScripts({ func: restoreTemplateReferences(deployTemplateMapping),
        isDc: client.isDataCenter,
        instances: changes
          .filter(isAdditionOrModificationChange)
          .filter(isInstanceChange)
