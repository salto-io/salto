...
const replaceRestVersion = (url: string): string => url.replace(CLOUD_REST_PREFIX, DATA_REST_CENTER_PREFIX)
...
const replaceToPluginUrl = (url: string, httpMethod: definitions.HTTPMethod): string | undefined =>
  PLUGIN_URL_PATTERNS.some(
    ({ httpMethods, url: patternUrl }) => httpMethods.includes(httpMethod) && createRegex(patternUrl).test(url),
  )
...
export const DATA_CENTER_SETTINGS: ProductSettings = {
  defaultApiDefinitions: definitions.mergeWithDefaultConfig(
