...
const getFieldReferences = async (
  ...
  if (columns === undefined || !Array.isArray(columns)) {
    log.warn(`columns value is corrupted in instance ${instance.elemID.getFullName()}, hence not calculating fields weak references`)
    return []
  }
 ...
 */
const removeMissingColumnsQueues: WeakReferencesHandler['removeWeakReferences'] = ({ elementsSource }) => async elements => {
  const fixedElements = await awu(elements)
    ...
    .map(async instance => {
      ...
      if (columns === undefined) {
        log.warn(`columns value is corrupted in instance ${instance.elemID.getFullName()}, hence not omitting missing fields`)
        return undefined
      }

      const fixedInstance = instance.clone()
      fixedInstance.value.columns = await awu(columns)
        .filter(async field => (field === undefined
          || (
            !isReferenceExpression(field)
