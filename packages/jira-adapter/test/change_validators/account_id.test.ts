...
describe('accountIdValidator', () => {
  ...
  const createWarning = ({
    ...
    severity: 'Warning',
    message:
      'The display name does not match the specified account ID. The element will be deployed with the appropriate display name instead.',
...
Go to ${url} to see valid users and account IDs.`,
  ...
  it('should return a warning when the display name is wrong', async () => {
    ...
    ).toEqual([createWarning({ elemId, parent, accountId, realDisplayName, currentDisplayName: 'wrong' })])
  })
  it('should not issue an error in permission scheme type with no account id', async () => {
    ...
    const permissionSchemeInstances = common.createInstanceElementArrayWithDisplayNames(1, objectType)
  ...
  it('should not raise errors when the flag is off', async () => {
    ...
    const validatorOff = accountIdValidator(client, configOff)
    const field1 = 'parameter'
    delete instances[0].value.holder[field1].displayName
    const field2 = 'accountId'
  ...
  describe('Using Jira DC', () => {
    ...
    const dcUsersInstance = new InstanceElement('users', usersType, {
      users: {
        JIRAUSER10000: {
    ...
    beforeEach(() => {
      validUserInstance = new InstanceElement('instance', objectType, {
      ...
      invalidUserInstance = new InstanceElement('instance', objectType, {
        notificationSchemeEvents: [
          {
            event: {
              id: '1',
    ...
    it('should not fail when user map func returns undefined', async () => {
      ...
      await expect(
        validator2([toChange({ after: instances[1] })], buildElementsSourceFromElements([])),
