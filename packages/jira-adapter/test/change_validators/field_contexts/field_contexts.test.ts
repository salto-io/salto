...
describe('Field contexts', () => {
  ...
  beforeEach(() => {
    ...
    contextInstance = new InstanceElement('instance', contextType, {
      isGlobalContext: false,
    })

    globalContextInstance = new InstanceElement('instance2', contextType, {
      isGlobalContext: true,
    })

    projectInstance = new InstanceElement('project_name', projectType, {
  ...
  it('should return an error when setting a context that is used in a project to be global', async () => {
    ...
    expect(await fieldContextValidator(changes, elementsSource)).toEqual([
      {
        elemID: globalContextInstance.elemID,
        severity: 'Error',
        message: 'Global field context can’t be referenced by a project.',
        detailedMessage:
          'This field context is global, but the following projects still reference it: project_name. Global field contexts can’t be referenced by projects. Please change this context to a non-global one, or add the projects without the reference to this deployment.',
      },
    ])
  })
  ...
  it('should return and error when creating a non global context without any other references to its field', async () => {
    ...
    elementsSource = buildElementsSourceFromElements(elements)
    projectInstance.value[PROJECT_CONTEXTS_FIELD] = []
    fieldInstance.value.contexts = [new ReferenceExpression(contextInstance.elemID)]
    expect(await fieldContextValidator(changes, elementsSource)).toEqual([
      {
  ...
  it('should not about a context if it is not in the changed elements', async () => {
    projectInstance.value[PROJECT_CONTEXTS_FIELD] = []
    expect(await fieldContextValidator([toChange({ after: projectInstance })], elementsSource)).toEqual([])
  })
  it('should not throw when one of the contexts is unresolved', async () => {
    ...
    await expect(fieldContextValidator(changes, elementsSource)).resolves.not.toThrow()
  })
  ...
  it('should warn when one of the project contexts is not a reference', async () => {
    ...
    expect(mockLogWarn).toHaveBeenCalledWith(
      'Found a non reference expression in project jira.Project.instance.project_name',
    )
  })
  it('should not return an error there are no changes', async () => {
    projectInstance.value[PROJECT_CONTEXTS_FIELD] = []
    expect(await fieldContextValidator([], elementsSource)).toEqual([])
  })
  it('should not return an error element source is not defined', async () => {
    projectInstance.value[PROJECT_CONTEXTS_FIELD] = []
    expect(await fieldContextValidator(changes, undefined)).toEqual([])
  })
})
