...
const projectError = (elemID: ElemID): ChangeError => ({
  ...
  detailedMessage:
    'This free Jira instance doesn’t support modifying associations between projects and permission schemes.',
})
...
const schemeError = (elemID: ElemID): ChangeError => ({
  ...
  detailedMessage:
    'The target Jira instance is a free one, which doesn’t support permission schemes. This change won’t be deployed.',
})
...
const schemeWarning = (elemID: ElemID): ChangeError => ({
  ...
  detailedMessage:
    'The target Jira instance is a free one, which doesn’t support permission schemes. This change won’t be deployed. The project will use a default change permission scheme instead.',
})
describe('permissionSchemeDeploymentValidator', () => {
  ...
  beforeEach(() => {
    ...
    permissionInstances = _.range(4).map(index => new InstanceElement(`permission${index}`, permissionType, {}))

    projectInstances = _.range(3).map(
      index =>
        new InstanceElement(`project${index}`, projectType, {
          permissionScheme: new ReferenceExpression(permissionInstances[index].elemID, {}),
        }),
    )
    project3NoScheme = new InstanceElement('project3', projectType, {})
    project0MoreFields = new InstanceElement('project0', projectType, {
  ...
  describe('projects', () => {
    it('should create a warning for project creation', async () => {
      expect(await validator([toChange({ after: projectInstances[0] })], elementsSource)).toEqual([
    ...
    it('should not create a warning for project creation without a scheme reference', async () => {
      expect(await validator([toChange({ after: project3NoScheme })], elementsSource)).toEqual([])
    })
    it('should create an error for association modification', async () => {
      expect(
        await validator([toChange({ before: projectInstances[1], after: projectInstances[0] })], elementsSource),
      ).toEqual([projectError(projectInstances[0].elemID)])
    })
    it('should create an error for association removal', async () => {
      expect(
        await validator([toChange({ before: projectInstances[1], after: project3NoScheme })], elementsSource),
      ).toEqual([projectError(project3NoScheme.elemID)])
    })
    it('should not create an error for removal', async () => {
      expect(await validator([toChange({ before: projectInstances[1] })], elementsSource)).toEqual([])
    })
    it('should not create an error for modification without change for association', async () => {
      expect(
        await validator([toChange({ before: project0MoreFields, after: projectInstances[0] })], elementsSource),
      ).toEqual([])
    })
  })
  describe('permission schemes', () => {
    describe('associated schemes', () => {
      ...
      it('should not return a warning for removal of project and scheme', async () => {
        ...
        expect(
          await validator(
            [toChange({ before: permissionInstances[0] }), toChange({ before: projectInstances[0] })],
            elementsSource,
          ),
        ).toEqual([])
      })
      it('should return a warning for creation of project and scheme', async () => {
        expect(
          await validator(
            [toChange({ after: permissionInstances[0] }), toChange({ after: projectInstances[0] })],
            elementsSource,
          ),
        ).toEqual([
          schemeWarning(permissionInstances[0].elemID),
          projectWarning(projectInstances[0].elemID, 'permission0'),
        ])
      })
      it('should return an error for schemes', async () => {
        expect(
          ...
          schemeError(permissionInstances[0].elemID),
    ...
    describe('unassociated schemes', () => {
      beforeEach(() => {
        elementsSource = buildElementsSourceFromElements([getAccountInfoInstance(true), ...permissionInstances])
      })
      it('should return an error for scheme creation', async () => {
        expect(await validator([toChange({ after: permissionInstances[0] })], elementsSource)).toEqual([
          schemeError(permissionInstances[0].elemID),
        ])
      })
  ...
  it('should not return an error for DC', async () => {
    ...
    expect(await dcValidator([toChange({ after: permissionInstances[0] })], elementsSource)).toEqual([])
  })
