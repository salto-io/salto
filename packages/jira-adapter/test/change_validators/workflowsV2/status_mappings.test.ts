...
describe('status mappings', () => {
  ...
  beforeEach(() => {
    jest.resetAllMocks()
    status1 = new InstanceElement(
      'status1',
      createEmptyType(STATUS_TYPE_NAME),
      {
        name: 'status1',
      }
    )
    status2 = new InstanceElement(
      'status2',
      createEmptyType(STATUS_TYPE_NAME),
      {
        name: 'status2',
  ...
  it('should return the correct error message when the workflow is the default workflow of the scheme', async () => {
    ...
    expect(await workflowStatusMappingsValidator(
      [toChange({ before, after: defaultWorkflowInstance })],
      elementsSource
    )).toEqual([{
