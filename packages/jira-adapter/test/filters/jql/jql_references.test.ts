...
describe('jqlReferencesFilter', () => {
  ...
  describe('onFetch', () => {
    ...
    it('should parse correctly jql with orderBy with unknown field', async () => {
      ...
      await filter.onFetch?.([instance, fieldInstance, doneInstance, todoInstance])
      expect(instance.value.jql).toBe('ORDER BY unknown ASC')
    })
    ...
    it('should parse correctly jql with customfields', async () => {
      ...
      await filter.onFetch?.([instance, fieldInstance, doneInstance, todoInstance, customField])
      expect(instance.value.jql).toBeInstanceOf(TemplateExpression)
      expect(instance.value.jql.parts).toEqual([
        '"',
    ...
    it('should not replace fields with escaping', async () => {
      ...
      await filter.onFetch?.([instance, fieldInstance, doneInstance, todoInstance, customField])
      expect(instance.value.jql).toBe('"a\\"b" = 2')
    })
