...
describe('generateTemplateExpression', () => {
  ...
  it('should parse correctly jql with orderBy', async () => {
    ...
    expect(expression).toEqual({
      template: new TemplateExpression({
        parts: ['ORDER BY ', new ReferenceExpression(fieldInstance.elemID, fieldInstance), ' ASC'],
      }),
      ambiguousTokens: new Set(),
    })
  })
  ...
  it('should not parse field in jql if there are another field with the same name', async () => {
    ...
    const expression = generateTemplateExpression(jql, generateJqlContext([anotherInstance, ...instances]))
    expect(expression).toEqual({
      template: undefined,
  ...
  it('should ignore functions', async () => {
    ...
    expect(expression).toEqual({
      template: new TemplateExpression({
        parts: [new ReferenceExpression(fieldInstance.elemID, fieldInstance), ' = currentUser()'],
      }),
      ambiguousTokens: new Set(),
    })
  })
