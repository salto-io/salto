...
import { ObjectType, ElemID, InstanceElement, BuiltinTypes, ReferenceExpression, Element, isInstanceElement } from '@salto-io/adapter-api'
...
describe('requestTypeRequestFormFilter', () => {
  ...
  it('should use elemIdGetter', async () => {
    ...
    const instances = elements.filter(isInstanceElement)
    const issueLayoutInstance = instances.find(e => e.elemID.typeName === REQUEST_FORM_TYPE)
    expect(issueLayoutInstance?.elemID.getFullName()).toEqual('jira.RequestForm.instance.requestTypeTest_Default_Request_Form@uss')
  })
})
