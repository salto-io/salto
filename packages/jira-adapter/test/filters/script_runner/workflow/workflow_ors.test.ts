...
describe('ScriptRunner ors in DC', () => {
  ...
  beforeEach(() => {
    ...
    >
    filterOff = orFilter(getFilterParams({ client, config: configOff })) as filterUtils.FilterWith<
      'onFetch' | 'preDeploy' | 'onDeploy'
    >
    filterCloud = orFilter(getFilterParams({ client: clientCloud, config })) as filterUtils.FilterWith<
      'onFetch' | 'preDeploy' | 'onDeploy'
    >
  ...
  describe('fetch', () => {
    ...
    it('should sort the order of the fields', async () => {
      instance.value.transitions.tran1.rules.postFunctions[0].configuration.FIELD_SELECTED_FIELDS =
        'reporter|||assignee'
      await filter.onFetch([instance])
      expect(instance.value.transitions.tran1.rules.postFunctions[0].configuration.FIELD_SELECTED_FIELDS).toEqual([
        'assignee',
        'reporter',
      ])
    })
    ...
    it('should not replace if wrong type', async () => {
      ...
      await filter.onFetch([instance])
      expect(instance.value.transitions.tran1.rules.postFunctions[1].configuration.FIELD_SELECTED_FIELDS).toEqual(
        'reporter|||assignee',
      )
    })
  })
