...
describe('workflowScheme', () => {
  ...
  describe('onFetch', () => {
    ...
    it('replace field issueTypeMappings with items', async () => {
      workflowSchemeType.fields.issueTypeMappings = new Field(
        workflowSchemeType,
        'issueTypeMappings',
        BuiltinTypes.STRING,
      )
  ...
  describe('deploy', () => {
    ...
    it('should update the id if changed', async () => {
      ...
      expect(connection.get).toHaveBeenCalledWith('/rest/api/3/workflowscheme/1', undefined)

      expect(connection.get).toHaveBeenCalledWith('/rest/api/3/workflowscheme', {
        headers: undefined,
    ...
    it('should reformat the error message when it does not get status migration', async () => {
      const workflowSchemeInstance = new InstanceElement('workflowSchemeInstance', workflowSchemeType, {
        workflow: 'workflow name',
      })
      ...
      deployChangeMock.mockResolvedValue({ draft: true })
      connection.post.mockImplementation(() => {
        throw new ServiceError(['Issue type with ID 2 is missing the mappings required for statuses with IDs 3,4'])
      })

      const instanceBefore = workflowSchemeInstance.clone()
      workflowSchemeInstance.value.workflow = 'other workflow'
      const result = await filter.deploy?.([toChange({ before: instanceBefore, after: workflowSchemeInstance })])
    ...
    it('should log the same error when the regex is not matched', async () => {
      ...
      elementsSource = buildElementsSourceFromElements([workflowSchemeInstance, statusFirstInstance, issueInstance])
      const { client: cli, paginator, connection: conn } = mockClient()
      client = cli
      connection = conn
      filter = workflowSchemeFilter(
      ...
      connection.post.mockImplementation(() => {
        throw new ServiceError([
          '<not correct message> 2 is missing the mappings required for statuses <not correct message> with 3',
        ])
      })
    ...
    it('should partly edit the error message when an ID is not in the elementsSource', async () => {
      ...
      elementsSource = buildElementsSourceFromElements([workflowSchemeInstance, issueInstance])
      const { client: cli, paginator, connection: conn } = mockClient()
      client = cli
      connection = conn
      filter = workflowSchemeFilter(
      ...
      deployChangeMock.mockResolvedValue({ draft: true })
      connection.post.mockImplementation(() => {
        throw new ServiceError(['Issue type with ID 2 is missing the mappings required for statuses with IDs 7'])
      })
