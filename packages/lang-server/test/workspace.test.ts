...
describe('workspace', () => {
  ...
  const withValidationErrorFileName = path.join(workspaceBaseDir, 'withValidationError.nacl')
  ...
  describe('validate files', () => {
    ...
    it('should not validate elements that are not already in the workspace', async () => {
      ...
      const errors = await newWorkspace.errors()
      expect(errors.validation).toHaveLength(0)
      const newErrors = await workspace.validateFiles([withValidationErrorFileName])
