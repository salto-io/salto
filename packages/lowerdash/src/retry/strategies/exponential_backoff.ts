...
): number => Math.floor(minInclusive + Math.random() * (maxInclusive - minInclusive + 1))

export default defaultOpts((opts: ExponentialBackoffOpts): RetryStrategy => {
  ...
  return () => {
    ...
    interval = (interval >= opts.max / opts.multiplier)
      ? opts.max
      : interval * opts.multiplier
    return result
  }
},
{
