...
export default class NetsuiteAdapter implements AdapterOperations {
  ...
  // private elementsIdByServiceId: Record<string, ElemID> | undefined
  ...
  public async fetch(): Promise<FetchResult> {
    const skipListQuery = notQuery(buildNetsuiteQuery({
      ...
      filePaths: this.filePathRegexSkipList,
    }))
    ...
    const instances = customizationInfos.map(customizationInfo => {
      ...
      return type && fetchQuery.isTypeMatch(type.elemID.name)
  ...
  // private async getElementsByServiceId() {
  //   if (this.elementsIdByServiceId === undefined) {
  //     for await (const element of await this.elementsSource.getAll()) {

  //     }
  //   }
  //   return this.elementsIdByServiceId
  // }
