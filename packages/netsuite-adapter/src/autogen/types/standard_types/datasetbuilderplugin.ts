...
export const datasetbuilderpluginType = (): TypeAndInnerTypes => {
  const innerTypes: Record<string, ObjectType> = {}

  const datasetbuilderpluginElemID = new ElemID(constants.NETSUITE, 'datasetbuilderplugin')

  const datasetbuilderplugin = new ObjectType({
    elemID: datasetbuilderpluginElemID,
    annotations: {},
    fields: {
      scriptid: {
    ...
    path: [constants.NETSUITE, constants.TYPES_PATH, datasetbuilderpluginElemID.name],
  })

  return { type: datasetbuilderplugin, innerTypes }
}
