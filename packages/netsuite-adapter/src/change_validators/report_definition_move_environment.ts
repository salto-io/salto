...
const getChangeError = async (instance: InstanceElement): Promise<ChangeError> => {
  if (await wasModified(instance)) {
    return ({
      ...
      severity: 'Error',
      message: 'Modified report definitions cannot be deployed.',
  ...
  return ({
    elemID: instance.elemID,
    severity: 'Warning',
    message: 'Beware that report definitions might reference internal ids that are not correct for the current environment. It is recommended that you verify the deployment in NetSuite UI.',
