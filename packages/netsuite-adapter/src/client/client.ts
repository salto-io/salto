...
export default class NetsuiteClient {
  ...
  private async listFilePaths(executor: CommandActionExecutor, query: NetsuiteQuery):
    Promise<{ listedPaths: string[]; failedPaths: string[] }> {
    const failedPaths: string[] = []
    const actionResults = (await Promise.all(
      fileCabinetTopLevelFolders
        .filter(folder => query.isFileMatch(folder))
        .map(folder =>
  ...
  async importFileCabinetContent(query: NetsuiteQuery):
    ...
    const listFilesResults = await this.listFilePaths(project.executor, query)
