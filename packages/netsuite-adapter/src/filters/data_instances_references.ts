...
const generateReference = (
  ...
  if (type && elementsMap[getDataInstanceId(value.internalId, type.elemID.name)]) {
    return new ReferenceExpression(elementsMap[getDataInstanceId(value.internalId, type.elemID.name)])
  }
  if (elementsMap[getDataInstanceId(value.internalId, value[TYPE_ID])]) {
    return new ReferenceExpression(elementsMap[getDataInstanceId(value.internalId, value[TYPE_ID])])
  }
  return undefined
}
...
const generateParentReference = (
  ...
  elementsMap[getDataInstanceId(value.internalId, type.elemID.name)]
    ? new ReferenceExpression(elementsMap[getDataInstanceId(value.internalId, type.elemID.name)])
    : undefined

const replaceReference =
  (type: ObjectType, elementsMap: Record<string, ElemID>): TransformFunc =>
  async ({ value, path, field }) => {
    if (path?.isTopLevel()) {
      return value
    }
    ...
    if (isListType(fieldType) && value.recordRef !== undefined) {
      return Promise.all(
        value.recordRef.map(
          async (val: Value) => generateReference(val, await fieldType.getInnerType(), elementsMap) ?? val,
        ),
      )
    }
...
const filterCreator: LocalFilterCreator = ({ elementsSourceIndex, isPartial }) => ({
  ...
  onFetch: async elements => {
    const elementsMap: Record<string, ElemID> = isPartial
      ? _.clone((await elementsSourceIndex.getIndexes()).internalIdsIndex ?? {})
      : {}
              ...
              if (Array.isArray(value) && field?.annotations.isReference) {
                return {
                  'platformCore:recordRef': value.map(val =>
                    isReferenceExpression(val) ? { attributes: { internalId: getReferenceInternalId(val) } } : val,
                  ),
                }
              }
              return value
