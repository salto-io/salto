...
const FIELD_TYPES_TO_OMIT = ['never', 'help', 'inlinehtml', 'label', 'text', 'textarea']
...
const isSelectFieldType = (fieldType: TypeElement): boolean => fieldType.elemID.typeName === SELECT_OPTION
...
const transformType = (configType: ObjectType, selectOptionType: ObjectType): void => {
  ...
  configType.fields = _(configType.annotations.fieldsDef)
    ...
    .mapValues(({ id, label, type }) => new Field(configType, id, toFieldType(type), { label, type }))
    .value()
...
const transformPrimitive = (value: Value, primitive: PrimitiveTypes, field: Field): Value => {
  switch (primitive) {
...
const getConfigInstanceValue = async (
  ...
  return mapValuesAsync(values, async (value, fieldName) => {
    ...
    return isPrimitiveType(fieldType) ? transformPrimitive(value, fieldType.primitive, field) : undefined
  })
}

const transformValuesForDeploy = async (change: Change<InstanceElement>): Promise<void> =>
  awu(Object.values(change.data)).forEach(async instance => {
...
const filterCreator: LocalFilterCreator = () => ({
  ...
  onFetch: async elements => {
    const [selectOptionType] = elements.filter(isObjectType).filter(type => type.elemID.name === SELECT_OPTION)
    ...
    await awu(elements)
      ...
      .forEach(async instance => {
        instance.value = await getConfigInstanceValue(instance.value.configRecord, await instance.getType())
      })
  },
