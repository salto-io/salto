...
const transformAnnotationsAndValues = async (element: Element, transformFunc: TransformFunc): Promise<void> => {
...
const filterCreator: LocalFilterCreator = () => ({
      ...
      .forEach(async element => {
        await transformAnnotationsAndValues(element, ({ value }) => {
          ...
          _(value)
            ...
            .forEach(([key, val]) => {
              value[getTranslateFieldName(key)] = val[TRANSLATE_KEY] === true || val[TRANSLATE_KEY] === 'T'
  ...
  preDeploy: async changes => {
    const elementsToTransform = changes
      .map(getChangeData)
      .filter(element => isInstanceElement(element) || (isObjectType(element) && isCustomRecordType(element)))
    await awu(elementsToTransform).forEach(async element => {
      await transformAnnotationsAndValues(element, ({ value }) => {
        if (!_.isPlainObject(value)) {
          return value
        }
