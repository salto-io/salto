...
describe('Currency changes change  validator', () => {
  ...
  describe('Additon changes to currency', () => {
    it('should have changeError when deploying a new currency with \'overrideCurrencyFormat\' disabled.', async () => {
      ...
      expect(changeErrors[0].detailedMessage).toContain('Cannot deploy currency - override currency format is disabled. Please see https://docs.salto.io/docs/netsuite#deploy-troubleshooting for instructions')
    })
  ...
  describe('Modification changes to currency', () => {
    ...
    it('should have changeError when editing displaySymbol or symbolPlacement with overrideCurrencyFormat disabled', async () => {
      ...
      expect(changeErrors[0].severity).toEqual('Error')
      expect(changeErrors[0].elemID).toEqual(instance.elemID)
      expect(changeErrors[0].detailedMessage).toContain('Cannot deploy currency - field display symbol cannot be edited. To enable editing this field, enable override currency format and try again. Please see https://docs.salto.io/docs/netsuite#deploy-troubleshooting for instructions')
    })
    ...
    it('should have changeError when modifying currencyPrecision', async () => {
      ...
      expect(changeErrors[0].severity).toEqual('Error')
      expect(changeErrors[0].elemID).toEqual(instance.elemID)
      expect(changeErrors[0].detailedMessage).toContain('Cannot deploy currency - currency precision is a read-only field in NetSuite. Please see https://docs.salto.io/docs/netsuite#deploy-troubleshooting for instructions')
    })
  })
})
