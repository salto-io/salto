...
describe('netsuite system note author information', () => {
  ...
  it('should query information from api', async () => {
    ...
    const fieldSystemNotesQuery = `SELECT name, field, recordid, ${toSuiteQLSelectDateString('date')} AS date From systemnote WHERE date >= TO_DATE('2022-1-1', 'YYYY-MM-DD') AND (field LIKE 'MEDIAITEM.%' OR field LIKE 'MEDIAITEMFOLDER.%') GROUP BY name, field, recordid, ${toSuiteQLSelectDateString('date')} ORDER BY name, field, recordid, date ASC`
