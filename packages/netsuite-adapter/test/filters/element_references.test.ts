...
describe.each([false, true])(
  'instance_references filter - useNewReferencesInSuiteScripts: %s',
  useNewReferencesInSuiteScripts => {
    describe('onFetch', () => {
      ...
      beforeEach(async () => {
        ...
        filterOpts.config.fetch.useNewReferencesInSuiteScripts = useNewReferencesInSuiteScripts
      ...
      it('should replace path references', async () => {
        await filterCreator(filterOpts).onFetch?.([fileInstance, workflowInstance, instanceWithRefs])
      ...
      it('should replace references in custom record type', async () => {
        await filterCreator(filterOpts).onFetch?.([customSegmentInstance, customRecordType])
      ...
      it('should not replace reference to hidden element', async () => {
        await filterCreator(filterOpts).onFetch?.([lockedCustomRecordType, instanceWithRefs])
      ...
      it('should use elements source for creating the references with fetch is partial', async () => {
        ...
        await filterCreator({ ...filterOpts, isPartial: true }).onFetch?.([
          fileInstance,
          workflowInstance,
          instanceWithRefs,
        ])
      ...
      it('should create _genereated_dependencies annotation and not replace the value in complexed values', async () => {
        await filterCreator(filterOpts).onFetch?.([customSegmentInstance, workflowInstance, instanceWithRefs])
      ...
      it('should not replace complexed strings and ignore non existing refs', async () => {
        await filterCreator(filterOpts).onFetch?.([instanceWithRefs])
      ...
      define(['N/record', '../SuiteScripts/oauth_1.js', '../SuiteScripts/oauth_2'], function(record) {
        ...
        filterOpts.config.fetch.calculateNewReferencesInSuiteScripts = true
        filterOpts.config.fetch.findReferencesInFilesWithExtension = ['.name']
        ...
        expect(mockLogInfo).toHaveBeenCalledTimes(useNewReferencesInSuiteScripts ? 0 : 1)
        ...
        expect(fileInstance.annotations[CORE_ANNOTATIONS.GENERATED_DEPENDENCIES]).toEqual(
          expect.arrayContaining([
            ...
            useNewReferencesInSuiteScripts
              ? {
                  // cseg_1 is an object key
                  reference: new ReferenceExpression(customSegmentInstance.elemID.createNestedID(SCRIPT_ID)),
                  occurrences: undefined,
                }
              : {
                  // custom_field is in a comment in the script content
                  reference: new ReferenceExpression(
      ...
      `
        ...
        await filterCreator(filterOpts).onFetch?.([innerFileInstance, workflowInstance])
        if (useNewReferencesInSuiteScripts) {
          expect(innerFileInstance.annotations[CORE_ANNOTATIONS.GENERATED_DEPENDENCIES]).toBeUndefined()
        } else {
    ...
    `
        ...
        await filterCreator(filterOpts).onFetch?.([fileInstance, noExtensionInstance, fileWithExtension])
     ...
     `
        ...
        await filterCreator(filterOpts).onFetch?.([fileInstance, commentRefFileInstance])
      ...
      define(['N/record', function(record) {
        ...
        fileInstance.value[PATH] = '/Templates/file.js'
        fileInstance.value.content = new StaticFile({ filepath: 'somePath', content: Buffer.from(fileContent) })
        await filterCreator(filterOpts).onFetch?.([fileInstance, customRecordType])
        expect(fileInstance.annotations[CORE_ANNOTATIONS.GENERATED_DEPENDENCIES]).toEqual([
          {
      ...
      define(['N/record', function(record) {
        ...
        fileInstance.value[PATH] = '/Templates/file.js'
        fileInstance.value.content = new StaticFile({ filepath: 'somePath', content: Buffer.from(fileContent) })
        await filterCreator({ ...filterOpts, isPartial: true }).onFetch?.([fileInstance])
        expect(fileInstance.annotations[CORE_ANNOTATIONS.GENERATED_DEPENDENCIES]).toEqual([
          {
