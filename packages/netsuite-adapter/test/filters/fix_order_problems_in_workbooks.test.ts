...
import { CHARTS, DATASETS, DATASET_LINK, DATASET_LINKS, DEPENDENCIES, DEPENDENCY, PIVOTS } from '../../src/type_parsers/analytics_parsers/analytics_constants'
...
describe('analytics definition handle filter', () => {
  ...
  const originOldWorkbook = new InstanceElement(
    'custworkbook_order_test',
    workbookType,
    {
      name: 'order test',
  ...
  const originWorkbook = new InstanceElement(
    'custworkbook_order_test',
    workbookType,
    {
      name: 'order test',
  ...
  const workbookForDeploy = new InstanceElement(
    'custworkbook_dependencies_test',
    workbookType,
    {
  ...
  const workbookForDeploywith3Datasets = new InstanceElement(
    'custworkbook_dependencies_test',
    workbookType,
    {
      [constants.SCRIPT_ID]: 'custworkbook_dependencies_test',
  ...
  const oldOddWorkbook = new InstanceElement(
    'custworkbook_with_odd_fields',
    workbookType,
    {
      name: 'order test',
  ...
  const dataset1ForDeploy = new InstanceElement(
    'custdataset_1',
    datasetType,
    {
  ...
  const dataset2ForDeploy = new InstanceElement(
    'custdataset_2',
    datasetType,
    {
  ...
  const dataset3ForDeploy = new InstanceElement(
    'custdataset_3',
    datasetType,
    {
  ...
  describe('onFetch', () => {
    ...
    it('should sort the order of the datasets in the pivot', async () => {
      ...
      const newWorkbook = elements.filter(isInstanceElement)
        .filter(e => e.elemID.typeName === constants.WORKBOOK)[0]
