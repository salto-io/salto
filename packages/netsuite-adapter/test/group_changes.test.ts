...
describe('Group Changes without Salto suiteApp', () => {
  ...
  const customFieldInstance = new InstanceElement('elementName', entitycustomfield)

  const customFieldFromSuiteAppInstance = new InstanceElement('elementNameFromSuiteApp', entitycustomfield, {
  ...
  const fileInstance = new InstanceElement('fileInstance', file, { path: '/SuiteScripts/a.txt' })

  const nonSdfFileInstance = new InstanceElement('fileInstance2', file, { path: '/not/allowed/path' })
  ...
  beforeAll(async () => {
    changeGroupIds = (
      await getChangeGroupIdsFunc(false)(
        new Map<string, Change>([
          ...
          [customFieldFromSuiteAppInstance.elemID.getFullName(), toChange({ after: customFieldFromSuiteAppInstance })],
          [customRecordTypeFromSuiteApp.elemID.getFullName(), toChange({ after: customRecordTypeFromSuiteApp })],
          [
  ...
  it('should set correct group id for custom types instances', () => {
    expect(changeGroupIds.get(customFieldInstance.elemID.getFullName())).toEqual(SDF_CREATE_OR_UPDATE_GROUP_ID)
  })

  it('should set correct group id for custom types elements from suiteapps', () => {
    expect(changeGroupIds.get(customFieldFromSuiteAppInstance.elemID.getFullName())).toEqual(
      `${SDF_CREATE_OR_UPDATE_GROUP_ID} - a.b.c`,
    )
    expect(changeGroupIds.get(customRecordTypeFromSuiteApp.elemID.getFullName())).toEqual(
      `${SDF_CREATE_OR_UPDATE_GROUP_ID} - a.b.c`,
    )
    expect(changeGroupIds.get(customRecordTypeFromSuiteApp.fields.custom_field.elemID.getFullName())).toEqual(
      `${SDF_CREATE_OR_UPDATE_GROUP_ID} - a.b.c`,
    )
  })
...
describe('Group Changes with Salto suiteApp', () => {
  ...
  const suiteAppFileInstance1 = new InstanceElement('fileInstance', file, {
    path: '/Images/file',
    content: new StaticFile({ filepath: 'somePath', content: Buffer.from('aaa') }),
  })

  const suiteAppFileInstance2 = new InstanceElement('fileInstance2', file, {
    path: '/Templates/file',
    content: new StaticFile({ filepath: 'somePath', content: Buffer.from('aaa') }),
  })

  const suiteAppFileInstance3Before = new InstanceElement('fileInstance3', file, {
    path: '/Images/file3',
    content: new StaticFile({ filepath: 'somePath', content: Buffer.from('aaa') }),
  })

  const suiteAppFileInstance3After = new InstanceElement('fileInstance3', file, {
    path: '/Images/file3',
    description: 'aa',
    content: new StaticFile({ filepath: 'somePath', content: Buffer.from('aaa') }),
  })

  const deletedSuiteAppFileInstance = new InstanceElement('deletedInstance4', file, {
    path: '/Images/file4',
    description: 'aa',
    content: new StaticFile({ filepath: 'somePath', content: Buffer.from('aaa') }),
  })

  const sdfFileInstance1 = new InstanceElement('fileInstance4', file, {
    path: '/Templates/file',
    content: new StaticFile({ filepath: 'somePath', content: Buffer.from('a'.repeat(11 * 1024 * 1024)) }),
  })
  const sdfFileInstance2 = new InstanceElement('fileInstance5', file, {
    path: '/Templates/file',
  ...
  const newDataInstance1 = new InstanceElement('newDataInstance1', subsidiaryType)
  const newDataInstance2 = new InstanceElement('newDataInstance2', subsidiaryType, {
    value: new ReferenceExpression(newDataInstance1.elemID),
  })
  const newDataInstance3 = new InstanceElement('newDataInstance3', subsidiaryType)
  const newDataInstance4 = new InstanceElement('newDataInstance4', subsidiaryType, {
    value: new ReferenceExpression(newDataInstance3.elemID),
  })

  const modifiedDataInstance = new InstanceElement('modifiedDataInstance', subsidiaryType)

  const deletedDataInstance1 = new InstanceElement('deletedDataInstance1', subsidiaryType)

  const deletedDataInstance2 = new InstanceElement('deletedDataInstance2', subsidiaryType, {
    value: new ReferenceExpression(deletedDataInstance1.elemID),
  })

  const deletedDataInstance3 = new InstanceElement('deletedDataInstance3', subsidiaryType)

  const deletedDataInstance4 = new InstanceElement('deletedDataInstance4', subsidiaryType, {
  ...
  beforeAll(async () => {
    changeGroupIds = (
      await getChangeGroupIdsFunc(true)(
        new Map<string, Change>([
          [customFieldInstance.elemID.getFullName(), toChange({ after: customFieldInstance })],
          [customFieldFromSuiteAppInstance.elemID.getFullName(), toChange({ after: customFieldFromSuiteAppInstance })],
          [nonSdfInstance.elemID.getFullName(), toChange({ after: nonSdfInstance })],
          ...
          [deletedSuiteAppFileInstance.elemID.getFullName(), toChange({ before: deletedSuiteAppFileInstance })],
          ...
          [
            modifiedDataInstance.elemID.getFullName(),
            toChange({
  ...
  it('should set correct group id for new suiteApp file instances', () => {
    expect(changeGroupIds.get(suiteAppFileInstance1.elemID.getFullName())).toEqual(SUITEAPP_CREATING_FILES_GROUP_ID)

    expect(changeGroupIds.get(suiteAppFileInstance2.elemID.getFullName())).toEqual(SUITEAPP_CREATING_FILES_GROUP_ID)
  })

  it('should set correct group id for existing suiteApp file instances', () => {
    expect(changeGroupIds.get(suiteAppFileInstance3Before.elemID.getFullName())).toEqual(
      SUITEAPP_UPDATING_FILES_GROUP_ID,
    )
  })

  it('should set correct group id for removed suiteApp file instances', () => {
    expect(changeGroupIds.get(deletedSuiteAppFileInstance.elemID.getFullName())).toEqual(
      SUITEAPP_DELETING_FILES_GROUP_ID,
    )
  })

  it('should set correct group id for SDF file instances', () => {
    expect(changeGroupIds.get(sdfFileInstance1.elemID.getFullName())).toEqual(SDF_CREATE_OR_UPDATE_GROUP_ID)

    expect(changeGroupIds.get(sdfFileInstance2.elemID.getFullName())).toEqual(SDF_CREATE_OR_UPDATE_GROUP_ID)
  })

  it('should set group chunks based on dependencies to data instances additions', () => {
    expect(changeGroupIds.get(newDataInstance1.elemID.getFullName())).toEqual(
      `${SUITEAPP_CREATING_RECORDS_GROUP_ID} - 1/2`,
    )

    expect(changeGroupIds.get(newDataInstance2.elemID.getFullName())).toEqual(
      `${SUITEAPP_CREATING_RECORDS_GROUP_ID} - 2/2`,
    )

    expect(changeGroupIds.get(newDataInstance3.elemID.getFullName())).toEqual(
      `${SUITEAPP_CREATING_RECORDS_GROUP_ID} - 1/2`,
    )

    expect(changeGroupIds.get(newDataInstance4.elemID.getFullName())).toEqual(
      `${SUITEAPP_CREATING_RECORDS_GROUP_ID} - 2/2`,
    )
  })
  it('should set group chunks based on dependencies to data instances deletions', () => {
    expect(changeGroupIds.get(deletedDataInstance1.elemID.getFullName())).toEqual(
      `${SUITEAPP_DELETING_RECORDS_GROUP_ID} - 2/2`,
    )

    expect(changeGroupIds.get(deletedDataInstance2.elemID.getFullName())).toEqual(
      `${SUITEAPP_DELETING_RECORDS_GROUP_ID} - 1/2`,
    )

    expect(changeGroupIds.get(deletedDataInstance3.elemID.getFullName())).toEqual(
      `${SUITEAPP_DELETING_RECORDS_GROUP_ID} - 2/2`,
    )

    expect(changeGroupIds.get(deletedDataInstance4.elemID.getFullName())).toEqual(
      `${SUITEAPP_DELETING_RECORDS_GROUP_ID} - 1/2`,
    )
  })
  it('should set correct group id for data instances modifications', () => {
    expect(changeGroupIds.get(modifiedDataInstance.elemID.getFullName())).toEqual(SUITEAPP_UPDATING_RECORDS_GROUP_ID)
  })

  it('should set correct group id for config instances', () => {
    expect(changeGroupIds.get(configInstance.elemID.getFullName())).toEqual(SUITEAPP_UPDATING_CONFIG_GROUP_ID)
  })
  ...
  it('should set correct group id for custom record type', () => {
    expect(changeGroupIds.get(deletedCustomRecordType.elemID.getFullName())).toEqual(SDF_DELETE_GROUP_ID)
  })

  it('should set correct group id for instance with non custom object type', () => {
    expect(changeGroupIds.get(deletedStandardInstance.elemID.getFullName())).toEqual(SDF_DELETE_GROUP_ID)
  })
})
