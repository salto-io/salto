...
describe('reference dependencies', () => {
  ...
  const dependsOn1Instance = new InstanceElement('dependsOn1Instance', entitycustomfield, {
    ...
    label: new ReferenceExpression(fileInstance.elemID.createNestedID(PATH), fileInstance.value[PATH], fileInstance),
  })

  const instance = new InstanceElement('elementName', entitycustomfield, {
    label: 'elementName',
  ...
  const anotherAdapterInstance = new InstanceElement(
    ...
    new ObjectType({
      elemID: new ElemID('another', 'type'),
      fields: {
  ...
  const instanceWithManyRefs = new InstanceElement('dependsOn2Instances', entitycustomfield, {
    ...
    label: new ReferenceExpression(
      dependsOn1Instance.elemID.createNestedID(SCRIPT_ID),
      dependsOn1Instance.value[SCRIPT_ID],
      dependsOn1Instance,
    ),
    description: new ReferenceExpression(instance.elemID.createNestedID('label'), instance.value.label, instance),
    help: new ReferenceExpression(
      anotherAdapterInstance.elemID.createNestedID('id'),
      anotherAdapterInstance.value.id,
      anotherAdapterInstance,
    ),
  })
  ...
  describe('getRequiredReferencedInstances', () => {
    ...
    beforeEach(() => {
      ...
      customSegmentInstance = new InstanceElement('customSegmentInstance', customsegment, {
        [SCRIPT_ID]: 'cseg_my_script_id',
      })

      customSegmentInstance.value.recordtype = new ReferenceExpression(customRecordType.elemID, 'val', customRecordType)

      customRecordType.annotations.customsegment = new ReferenceExpression(
        customSegmentInstance.elemID,
        'val',
        customSegmentInstance,
      )

      datasetInstance = new InstanceElement('datasetInstance', dataset, {
        [SCRIPT_ID]: 'custdataset_my_script_id',
      })

      workbookInstance = new InstanceElement('workbookInstance', workbook, {
        [SCRIPT_ID]: 'custworkbook_my_script_id',
        dependencies: {
          dependency: new ReferenceExpression(datasetInstance.elemID, 'val', datasetInstance),
        },
      })
    })
    ...
    it('should not add dependencies that already exist', async () => {
      const input = [customRecordType, customSegmentInstance, workbookInstance, datasetInstance, instance]
    ...
    it('should not add new dependencies more then once', async () => {
      const customRecordType2 = new ObjectType({
        ...
        annotations: {
          [METADATA_TYPE]: CUSTOM_RECORD_TYPE,
          [SCRIPT_ID]: 'customrecord_my_script_id',
          customsegment: new ReferenceExpression(customSegmentInstance.elemID, 'val', customSegmentInstance),
        },
      })

      const result = await getReferencedElements([customRecordType, customRecordType2], false)
      expect(result).toEqual([customSegmentInstance])
    })
    ...
    it('should add translation collection instances when referenced', async () => {
      ...
      const translationCollectionInstanceReferencedInInstance = new InstanceElement(
        'custtranslation_test1',
        translationcollection,
      )
      const translationCollectionInstanceReferencedInType = new InstanceElement(
        'custtranslation_test2',
        translationcollection,
      )
