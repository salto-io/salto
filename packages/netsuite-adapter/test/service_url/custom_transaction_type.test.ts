...
describe('setCustomTransactionTypesUrls', () => {
  ...
  it('should set the right url', async () => {
    const elements = [
      new InstanceElement('A', customtransactiontype, { scriptid: 'someScriptId', [INTERNAL_ID]: '1' }),
    ]
    await setServiceUrl(elements, client)
    expect(elements[0].annotations[CORE_ANNOTATIONS.SERVICE_URL]).toBe('https://accountid.app.netsuite.com/app/common/custom/customtransaction.nl?id=1')
  })
