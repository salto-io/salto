...
describe('setCustomTransactionTypesUrls', () => {
  ...
  beforeEach(() => {
    ...
    elements = [
      new InstanceElement('A', customtransactiontype, { scriptid: 'someScriptId' }),
    ]
  })

  it('should set the right url', async () => {
    await setServiceUrl(elements, client)
    expect(elements[0].annotations[CORE_ANNOTATIONS.SERVICE_URL]).toBe('https://tstdrv2259448.app.netsuite.com/app/common/custom/customtransaction.nl?id=1')
  })
