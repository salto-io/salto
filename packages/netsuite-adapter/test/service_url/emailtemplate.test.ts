...
describe('setEmailTemplatesUrls', () => {
  ...
  it('should set the right url', async () => {
    const elements = [new InstanceElement('A', emailtemplate, { scriptid: 'someScriptId', [INTERNAL_ID]: '1' })]
    await setServiceUrl(elements, client)
    expect(elements[0].annotations[CORE_ANNOTATIONS.SERVICE_URL]).toBe(
      'https://accountid.app.netsuite.com/app/crm/common/merge/emailtemplate.nl?id=1&cp=F',
    )
  })
