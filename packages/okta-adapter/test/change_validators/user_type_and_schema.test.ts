...
describe('userTypeAndSchemaValidator', () => {
  ...
  const userTypeA = new InstanceElement(
    'typeA',
    userTypeType,
    { name: 'A', default: false },
  )
  const userTypeB = new InstanceElement(
    'typeB',
    userTypeType,
    { name: 'B', default: false },
  )
  const userSchemaA = new InstanceElement(
    'schemaA',
    userSchemaType,
    { name: 'A', definitions: {} },
    undefined,
  ...
  const userSchemaB = new InstanceElement(
    'schemaB',
    userSchemaType,
    { name: 'B', definitions: {} },
    undefined,
  ...
  it('should return an error when userSchema is deleted without its parent UserType', async () => {
    const changeErrors = await userTypeAndSchemaValidator(
      [toChange({ before: userSchemaA }), toChange({ before: userSchemaB })]
    )
