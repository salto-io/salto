...
import { InstanceElement, toChange, DependencyChange, ElemID, ObjectType, ReferenceExpression } from '@salto-io/adapter-api'
...
describe('changeDependenciesFromPoliciesAndRulesToPriority', () => {
  let dependencyChanges: DependencyChange[]
    it.each([...ALL_SUPPORTED_POLICY_RULE_NAMES, ...ALL_SUPPORTED_POLICY_NAMES])(
      ...
      it.each([...ALL_SUPPORTED_POLICY_RULE_NAMES, ...ALL_SUPPORTED_POLICY_NAMES])('should not add dependencies from %sPriority to its priority when %s is addition change', async (policyName: string) => {
        const policyType = new ObjectType({ elemID: new ElemID(OKTA, policyName) })
        const priorityType = new ObjectType({ elemID: new ElemID(OKTA, `${policyName}Priority`) })
      ...
      it.each([...ALL_SUPPORTED_POLICY_RULE_NAMES, ...ALL_SUPPORTED_POLICY_NAMES])('should not add dependencies from %sPriority to its priority when priority is addition change', async (policyName: string) => {
        const policyType = new ObjectType({ elemID: new ElemID(OKTA, policyName) })
        const priorityType = new ObjectType({ elemID: new ElemID(OKTA, `${policyName}Priority`) })
        const policyInstanceBefore = new InstanceElement('policyInstance', policyType, {
          id: '1',
      ...
      it.each([...ALL_SUPPORTED_POLICY_RULE_NAMES, ...ALL_SUPPORTED_POLICY_NAMES])('should not add dependencies from %sPriority to its priority when both are addition change', async (policyName: string) => {
        const policyType = new ObjectType({ elemID: new ElemID(OKTA, policyName) })
        const priorityType = new ObjectType({ elemID: new ElemID(OKTA, `${policyName}Priority`) })
        const policyInstanceAfter = new InstanceElement('policyInstance', policyType, {
          id: '1',
