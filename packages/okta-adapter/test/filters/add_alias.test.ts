...
describe('addAliasFilter', () => {
  ...
  const appAInstance = new InstanceElement(
    'app1',
    appType,
    {
      id: '1',
  ...
  const appBInstance = new InstanceElement(
    'app2',
    appType,
    {
      id: '2',
  ...
  const elements = [groupType, appType, profileMappingType, groupInstance, appAInstance,
    appBInstance, profileMappingType, mappingInstance]
  ...
  it('should add alias for instances', async () => {
    ...
    const aliases = elements
      ...
      .map(instance => instance.annotations[CORE_ANNOTATIONS.ALIAS]).sort()
