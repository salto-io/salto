...
describe('template static file', () => {
  ...
  const macro1 = new InstanceElement('macro1', new ObjectType({ elemID: new ElemID('zendesk', 'macro') }), { id: 2, actions: [{ value: 'non template', field: 'comment_value_html' }] })


  const singleLineTemplate = createTemplateExpression({ parts: [
  ...
  const singleLineWithTemplateMarkerTemplate = createTemplateExpression({ parts: [
    '/hc/test${/test/articles/',
    new ReferenceExpression(article.elemID),
  ...
  const singleLineRefAtBeginningAndEnd = createTemplateExpression({ parts: [
    new ReferenceExpression(article.elemID),
    '/hc/test/test/articles/',
    new ReferenceExpression(article.elemID),
    // eslint-disable-next-line no-template-curly-in-string
  ...
  const singleLineSpecialChars = createTemplateExpression({ parts: [
    '/hc/test/test/articles/',
    new ReferenceExpression(article.elemID),
    // eslint-disable-next-line no-template-curly-in-string
  ...
  const multiLineTemplate = createTemplateExpression({ parts: [
    '/hc/test/test/articles/',
    new ReferenceExpression(article.elemID),
    '\n/test hc/test/test/articles/',
    new ReferenceExpression(macro1.elemID),
  ...
  const multiLineWithTemplateMarkerTemplate = createTemplateExpression({ parts: [
    '/hc/test${/test/articles/',
    new ReferenceExpression(article.elemID),
    // eslint-disable-next-line no-template-curly-in-string
    '\n/test ${hc/test/test/articles/}',
  ...
  const multiLineRefAtBeginningAndEnd = createTemplateExpression({ parts: [
    new ReferenceExpression(article.elemID),
    '/hc/test/test/articles/',
    new ReferenceExpression(article.elemID),
    // eslint-disable-next-line no-template-curly-in-string
  ...
  const multiLineSpecialChars = createTemplateExpression({ parts: [
    '/hc/test/test/articles/',
    new ReferenceExpression(article.elemID),
    // eslint-disable-next-line no-template-curly-in-string
    '\nHe ${said, \'\'\' "Hello, World!"\tThis is a backslash: \\ 😄',
  ...
  describe('templateExpressionToStaticFile', () => {
    const testTemplateExpressionToStaticFile = async (templateExpression: TemplateExpression, stringResult: string)
      : Promise<void> => {
