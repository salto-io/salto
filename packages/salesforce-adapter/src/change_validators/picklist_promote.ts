...
const isGlobalPicklistChange = async (change: Change): Promise<boolean> => {
  ...
  return (
    isPicklistField(before) &&
    isPicklistField(after) &&
    isCustom(await apiName(before)) &&
    !hasValueSetNameAnnotation(before) &&
    hasValueSetNameAnnotation(after)
  )
}
