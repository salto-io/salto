...
import {
  ...
  isAdditionOrModificationChange, isObjectType, isField, ObjectType, Field,
...
const dataRecordToAssociatedType: DependencyChanger = async changes => {
  ...
  const typeChanges = Array.from(changes.entries())
    .filter(([, change]) => isCustomObjectChange(change))
  ...
  const typeElemIdToChangeIdMap = new Map<ElemID, ChangeId>(typeChanges
    .map(([changeId, change]) => (
      [getAffectedType(getChangeData(change) as Field | ObjectType).elemID, changeId]
    )))
