...
export const buildDataManagement = (params: DataManagementConfig): DataManagement => {
  ...
  return {
    shouldFetchObjectType: async objectType => {
      ...
      */
      ...
      const hasManagedBySaltoField =
        managedBySaltoFieldName !== undefined && objectType.fields[managedBySaltoFieldName] !== undefined
    ...
    brokenReferenceBehaviorForTargetType: typeName => {
      ...
      const typeOverrides =
        params.brokenOutgoingReferencesSettings?.perTargetTypeOverrides ?? DEFAULT_PER_TYPE_BROKEN_REFS_BEHAVIOR
    ...
    getObjectIdsFields: name => {
      const matchedOverride = params.saltoIDSettings.overrides?.find(override =>
        new RegExp(`^${override.objectsRegex}$`).test(name),
      )
    ...
    getObjectAliasFields: name => {
      ...
      const matchedOverride = params.saltoAliasSettings?.overrides?.find(override =>
        new RegExp(`^${override.objectsRegex}$`).test(name),
      )
    ...
    omittedFieldsForType: name => (name === undefined ? [] : makeArray(omittedFieldsByType[name])),
    regenerateSaltoIds: params.saltoIDSettings.regenerateSaltoIds ?? false,
  }
}
...
export const validateDataManagementConfig = (
  ...
  if (dataManagementConfig.saltoIDSettings.overrides !== undefined) {
    const overridesObjectRegexes = dataManagementConfig.saltoIDSettings.overrides.map(override => override.objectsRegex)
    validateRegularExpressions(overridesObjectRegexes, [...fieldPath, 'saltoIDSettings', 'overrides'])
  }
    ...
    Object.entries(dataManagementConfig.brokenOutgoingReferencesSettings.perTargetTypeOverrides).forEach(
      ([type, outgoingRefBehavior]) => {
        if (!outgoingReferenceBehaviors.includes(outgoingRefBehavior)) {
          throw new ConfigValidationError(
            [...fieldPath, 'brokenOutgoingReferencesSettings', 'perTargetTypeOverrides', type],
  ...
  const invalidOmittedFieldNames = makeArray(dataManagementConfig.omittedFields).filter(
    omittedFieldName => omittedFieldName.split('.').length < 2,
  )
