...
import { apiName, createInstanceElement, isNull, MetadataValues, XsdType } from '../transformers/transformer'
...
const isServiceMDTRecordFieldValue = (value: Values): value is ServiceMDTRecordFieldValue =>
  ...
  (value.value === undefined || isNull(value.value) || _.isString(value.value[`${XML_ATTRIBUTE_PREFIX}xsi:type`]))

const isServiceMDTRecordValues = (value: Values): value is ServiceMDTRecordValue =>
  'values' in value && makeArray(value.values).every(isServiceMDTRecordFieldValue)
...
const serviceFieldValueToNaclValue = (value: ServiceMDTRecordFieldValue): NaclMDTRecordFieldValue => ({
  field: value.field,
  ...(isNull(value.value)
    ? {}
    : {
...
const getCustomMetadataType = async (
  ...
  return awu(customMetadataTypes).find(async objectType => (await apiName(objectType)) === customMetadataTypeName)
}
...
const formatRecordValuesForService = async (instance: InstanceElement): Promise<Values> => {
