...
const SUPPORTED_FIELD_TYPE_NAMES = [
  ...
  FIELD_TYPE_NAMES.DATETIME,
  FIELD_TYPE_NAMES.PICKLIST,
  FIELD_TYPE_NAMES.TEXT,
  FIELD_TYPE_NAMES.PHONE,
  FIELD_TYPE_NAMES.TEXTAREA,
  FIELD_TYPE_NAMES.LONGTEXTAREA,
  FIELD_TYPE_NAMES.URL,
  FIELD_TYPE_NAMES.EMAIL,
  FIELD_TYPE_NAMES.NUMBER,
  FIELD_TYPE_NAMES.PERCENT,
] as const
...
const isServiceMDTRecordValues = (
  value: Values,
): value is ServiceMDTRecordValue =>
  'values' in value &&
...
const serviceFieldValueToNaclValue = (
  ...
  ...(isNull(value.value)
    ? {}
    : {
        value: _.get(value.value, '#text'),
        type: _.get(value.value, 'attr_xsi:type'),
      }),
})
...
const getCustomMetadataType = async (
  ...
  const customMetadataTypeName = (await apiName(instance))
    .split('.')[0]
    .concat(CUSTOM_METADATA_SUFFIX)
  return awu(customMetadataTypes).find(
    async (objectType) =>
      (await apiName(objectType)) === customMetadataTypeName,
  )
}

const extractValuesToFields = (recordValues: ServiceMDTRecordValue): Values =>
  Object.fromEntries(
...
const formatMDTRecordValuesToNacl = (values: ServiceMDTRecordValue): Values =>
  _.omit(
    ...
    'values',
    Object.keys(additionalNamespaces),
  )

const iSupportedFieldTypeName = (
  fieldTypeName: string,
): fieldTypeName is SupportedFieldTypeName =>
...
const getFieldXsdType = async (field: Field): Promise<XsdType | undefined> => {
  ...
  if (!iSupportedFieldTypeName(fieldTypeName)) {
    log.warn(
      'Unsupported field type %s on field %s',
      fieldTypeName,
...
const formatRecordValuesForService = async (
  ...
  return {
    ...
    ..._.omit(
      instance.value,
      INTERNAL_ID_FIELD,
      Object.keys(instance.value).filter((k) =>
        k.endsWith(SALESFORCE_CUSTOM_SUFFIX),
      ),
    ),
    values,
  }
...
const getInstanceWithCorrectType = async (
  ...
  const correctType = await getCustomMetadataType(
    instance,
    customMetadataRecordTypes,
  )
  if (_.isUndefined(correctType)) {
    log.warn(
      'Could not fix type for CustomMetadataType Instance %s, since its CustomMetadata record type was not found',
  ...
  if (!isMetadataValues(formattedValues)) {
    log.warn(
      'CustomMetadata instance %s is missing the fullName field, skipping.',
      instance.elemID.getFullName(),
    )
    return undefined
  }
...
const toDeployableChange = async (
  ...
  const deployableAfter = createInstanceElement(
    (await formatRecordValuesForService(
  ...
  return {
    ...
    onFetch: async (elements) => {
      ...
      const newInstances = await awu(oldInstances)
        .map((instance) =>
          getInstanceWithCorrectType(instance, customMetadataRecordTypes),
        )
        .filter(isDefined)
        .toArray()
    ...
    preDeploy: async (changes) => {
      const originalChanges = await awu(changes)
        ...
        .filter((change) =>
          isCustomMetadataRecordInstance(getChangeData(change)),
        )
        .toArray()
      originalChangesByApiName = await keyByAsync(originalChanges, (c) =>
        apiName(getChangeData(c)),
      )
      const deployableChanges = await awu(originalChanges)
        .map(toDeployableChange)
        .toArray()
    ...
    onDeploy: async (changes) => {
      const appliedChanges = await awu(changes)
        ...
        .filter(isAdditionOrModificationChange)
        .filter((change) =>
          isInstanceOfType(CUSTOM_METADATA)(getChangeData(change)),
        )
        .toArray()
