...
export const annotationsFileName = (objectName: string): string => `${pathSaltoCase(objectName)}Annotations`
export const standardFieldsFileName = (objectName: string): string => `${pathSaltoCase(objectName)}StandardFields`
export const customFieldsFileName = (objectName: string): string => `${pathSaltoCase(objectName)}CustomFields`

const getObjectDirectoryPath = (obj: ObjectType, namespace?: string): string[] => {
  const elemFileName = pathSaltoCase(obj.elemID.name)
  if (namespace) {
