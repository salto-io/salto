 ...
 */
import { Element, ElemID, Field, isInstanceElement, ReferenceExpression } from '@salto-io/adapter-api'
...
const filterCreator: LocalFilterCreator = ({ config }) => ({
  ...
  onFetch: async (elements: Element[]) => {
    ...
    const topLevelElementsReferences = elements
      ...
      .filter(
        (elementReference): elementReference is [Element, ReferenceExpression] => elementReference[1] !== undefined,
      )
    ...
    if (!config.fetchProfile.isCustomReferencesHandlerEnabled('managedElements')) {
      ...
      log.debug('Managed elements custom references handler enabled, not adding explicit references.')
    }
