...
const toNaclFilename = async (
  fieldName: string,
  objType: ObjectType,
): Promise<string> =>
  fieldName !== undefined &&
  isMapType(await objType.fields[fieldName]?.getType())
    ? strings.capitalizeFirstLetter(fieldName)
    : DEFAULT_NACL_FILENAME

const splitProfile = async (
  ...
  const profileInstances = await Promise.all(
    ...
    ).map(async ([fileName, fields]) => toInstancePart(fileName, fields)),
  )
...
const isProfileInstance = async (elem: Element): Promise<boolean> =>
  ...
  (await metadataType(elem)) === PROFILE_METADATA_TYPE
...
const filterCreator: LocalFilterCreator = () => ({
  ...
  onFetch: async (elements: Element[]) => {
    ...
    const newProfileInstances = await awu(profileInstances)
      .flatMap(splitProfile)
      .toArray()
