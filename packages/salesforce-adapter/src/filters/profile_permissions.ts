...
const createAdminProfile = (): InstanceElement =>
  createInstanceElement(
    {
...
const isAdminProfileChange = async (change: Change): Promise<boolean> => {
  ...
  return (
    (await isInstanceOfType(PROFILE_METADATA_TYPE)(changeElem)) &&
  ...
  return {
    ...
    preDeploy: async (changes) => {
      ...
      if (adminProfileChange === undefined) {
        ...
        changes.push({
          action: 'modify',
          data: { before: createAdminProfile(), after: adminProfile },
        })
      }
    },
