...
describe('SalesforceAdapter fetch', () => {
  ...
  const mockMetadataType = (
    ...
    if (instances !== undefined) {
      connection.metadata.list.mockImplementation(async queries => {
        const queryTypes = makeArray(queries).map(query => query.type)
        // Return the instances only for the queried types or their folder types
        if ([typeDef.xmlName, valueDef.parentField?.foreignKeyDomain ?? ''].some(type => queryTypes.includes(type))) {
          return instances.map(inst => mockFileProperties({ type: typeDef.xmlName, ...inst.props }))
        }
        return []
      })
  ...
  describe('when fetch is without changes detection', () => {
    beforeEach(() => {
      ({ connection, adapter } = mockAdapter({
        adapterParams: {
          getElemIdFunc: mockGetElemIdFunc,
