...
describe('Task or Event Fields Modifications Change Validator', () => {
  ...
  beforeEach(() => {
    ...
    taskFieldOfNonCustomObject = createField(new ObjectType({elemID: new ElemID(SALESFORCE, TASK_CUSTOM_OBJECT)}), BuiltinTypes.STRING, FIELD_NAME)
    
  })
  it('should create correct change errors', async () => {
    const errors = await changeValidator([taskField, eventField, taskFieldOfNonCustomObject].map(field => toChange({after: field})))
