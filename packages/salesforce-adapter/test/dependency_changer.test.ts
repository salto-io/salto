...
*/
import {
  ChangeId, Change, DependencyChange, InstanceElement, ObjectType, toChange, getChangeData,
...
const createChangeMapFromChanges = (changes: Change[]): Map<ChangeId, Change> => (
  new Map(
    changes.map(change => [createChangeId(change), change])
  )
)
...
describe('Dependency Changer', () => {
  ...
  beforeEach(() => {
    typeChange = toChange({ after: mockTypes.Account })
  })
  describe('if there is no matching type', () => {
    beforeEach(async () => {
      instanceChange = toChange({ after: new InstanceElement('testInstance', getChangeData(typeChange)) })
  ...
  describe('if the change is instance removal', () => {
    beforeEach(async () => {
      instanceChange = toChange({ before: new InstanceElement('testInstance', getChangeData(typeChange)) })
  ...
  describe('if the change is instance modification', () => {
    beforeEach(async () => {
      const instanceElement = new InstanceElement('testInstance', getChangeData(typeChange))
      instanceChange = toChange({ before: instanceElement, after: instanceElement })
      changes = createChangeMapFromChanges([instanceChange, typeChange])
      deps = Array.from(await dependencyChanger(changes, new Map()))
    })
    it('should create a dep', () => {
