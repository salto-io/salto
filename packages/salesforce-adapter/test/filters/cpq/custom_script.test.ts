...
describe('cpq custom script filter', () => {
  ...
  describe('onFetch', () => {
    beforeAll(async () => {
      elements = [mockCustomScriptObject.clone(), mockCustomScriptInstance.clone()]
    ...
    it('Should change fieldsRefList fields type to list of text', async () => {
      ...
      expect(await customScriptObj.fields[CPQ_CONSUMPTION_RATE_FIELDS].getType()).toEqual(
    ...
    it('Should only change values of multi-line string in fieldsRefList to array of strings and code to static file', () => {
      ...
      expect(customScriptInstance.value[CPQ_CODE_FIELD]).toEqual(afterOnFetchCustomScriptValues[CPQ_CODE_FIELD])
      expect(customScriptInstance.value[CPQ_GROUP_FIELDS]).toEqual(fromServiceCustomScriptValues[CPQ_GROUP_FIELDS])
      expect(customScriptInstance.value.randomField).toEqual(fromServiceCustomScriptValues.randomField)
    })
  })
  ...
  describe('onDeploy', () => {
    ...
    describe('Addition changes', () => {
      ...
      it('Should change fieldsRefList fields type to list of text', async () => {
        ...
        expect(await object.fields[CPQ_CONSUMPTION_RATE_FIELDS].getType()).toEqual(
          new ListType(Types.primitiveDataTypes.Text),
        )
        expect(await object.fields[CPQ_GROUP_FIELDS].getType()).toEqual(new ListType(Types.primitiveDataTypes.Text))
      })
      ...
      it('Should only change values of multi-line string in fieldsRefList to array of strings', () => {
        const customScriptInstanceAdd = changes.find(
          change => getChangeData(change).elemID.isEqual(mockCustomScriptInstance.elemID) && isAdditionChange(change),
        )
        expect(customScriptInstanceAdd).toBeDefined()
        const changeData = getChangeData(customScriptInstanceAdd as Change) as InstanceElement
        expect(changeData.value[CPQ_CONSUMPTION_RATE_FIELDS]).toEqual(
          afterOnFetchCustomScriptValues[CPQ_CONSUMPTION_RATE_FIELDS],
        )
        expect(fromServiceCustomScriptValues[CPQ_CODE_FIELD]).toEqual(fromServiceCustomScriptValues[CPQ_CODE_FIELD])
        expect(changeData.value[CPQ_GROUP_FIELDS]).toEqual(fromServiceCustomScriptValues[CPQ_GROUP_FIELDS])
        expect(changeData.value.randomField).toEqual(fromServiceCustomScriptValues.randomField)
      })
    })
    ...
    describe('Modification changes', () => {
      ...
      it('should change field type to list of text for modified fields', async () => {
        ...
        expect(await cpqFieldChange.data.before.getType()).toEqual(new ListType(Types.primitiveDataTypes.Text))
        expect(await cpqFieldChange.data.after.getType()).toEqual(new ListType(Types.primitiveDataTypes.Text))
      })
      ...
      it('Should only change values of multi-line string in fieldsRefList to array of strings', () => {
        const customScriptInstanceModify = changes.find(
          change =>
            getChangeData(change).elemID.isEqual(mockCustomScriptInstance.elemID) && isModificationChange(change),
        )
        expect(customScriptInstanceModify).toBeDefined()
        const changeData = getChangeData(customScriptInstanceModify as Change) as InstanceElement
        expect(changeData.value[CPQ_CONSUMPTION_RATE_FIELDS]).toEqual(
          afterOnFetchCustomScriptValues[CPQ_CONSUMPTION_RATE_FIELDS],
        )
        expect(changeData.value[CPQ_CODE_FIELD]).toEqual(fromServiceCustomScriptValues[CPQ_CODE_FIELD])
        expect(changeData.value[CPQ_GROUP_FIELDS]).toEqual(fromServiceCustomScriptValues[CPQ_GROUP_FIELDS])
        expect(changeData.value.randomField).toEqual(fromServiceCustomScriptValues.randomField)
      })
    })
  ...
  describe('preDeploy', () => {
    ...
    mockAfterOnFetchCustomScriptObject.fields[CPQ_CONSUMPTION_RATE_FIELDS].refType = createRefToElmWithValue(
      new ListType(Types.primitiveDataTypes.Text),
    )
    mockAfterOnFetchCustomScriptObject.fields[CPQ_GROUP_FIELDS].refType = createRefToElmWithValue(
      new ListType(Types.primitiveDataTypes.Text),
    )
    const mockAfterResolveCustomScriptInstance = mockAfterOnFetchCustomScriptInstance.clone()
    // Simulating resolve on the static-file
    mockAfterResolveCustomScriptInstance.value[CPQ_CODE_FIELD] = cpqCodeContent.toString('utf-8')
    describe('Modification changes', () => {
      beforeAll(async () => {
      ...
      it('Should change fieldRefList fields type to long text', async () => {
        const cpqFieldChange = changes.find(isFieldChange) as ModificationChange<Field>
        expect(cpqFieldChange).toBeDefined()
        expect(await cpqFieldChange.data.before.getType()).toEqual(Types.primitiveDataTypes.LongTextArea)
        expect(await cpqFieldChange.data.after.getType()).toEqual(Types.primitiveDataTypes.LongTextArea)
      })
      ...
      it('Should only change values of multi-line string in fieldsRefList', () => {
        ...
        expect(customScriptInstanceModify).toBeDefined()
        const afterElement = (customScriptInstanceModify as ModificationChange<InstanceElement>).data.after
        expect(afterElement.value[CPQ_CONSUMPTION_RATE_FIELDS]).toEqual('lala\nzaza\nbaba')
        expect(afterElement.value[CPQ_CODE_FIELD]).toEqual(fromServiceCustomScriptValues[CPQ_CODE_FIELD])
        expect(afterElement.value[CPQ_GROUP_FIELDS]).toEqual(fromServiceCustomScriptValues[CPQ_GROUP_FIELDS])
        expect(afterElement.value.randomField).toEqual(fromServiceCustomScriptValues.randomField)
        const beforeElement = (customScriptInstanceModify as ModificationChange<InstanceElement>).data.before
        expect(beforeElement.value[CPQ_CONSUMPTION_RATE_FIELDS]).toEqual('lala\nzaza\nbaba')
        expect(beforeElement.value[CPQ_CODE_FIELD]).toEqual(fromServiceCustomScriptValues[CPQ_CODE_FIELD])
        expect(beforeElement.value[CPQ_GROUP_FIELDS]).toEqual(fromServiceCustomScriptValues[CPQ_GROUP_FIELDS])
        expect(beforeElement.value.randomField).toEqual(fromServiceCustomScriptValues.randomField)
      })
    })
    ...
    describe('Addition changes', () => {
      ...
      it('Should change fieldsRefList fields type to long text on addition change', async () => {
        ...
        expect(await object.fields[CPQ_GROUP_FIELDS].getType()).toEqual(Types.primitiveDataTypes.LongTextArea)
      })
