  ...
  describe('onFetch', () => {
    ...
    describe('Object lookup with valuesMapping defined', () => {
      it('Should change lookup field value set fullName value to a ref if custom object exists (mapped and not) and keep string if not', () => {
        const mockConfigurationAttr = elements.find(e => e.elemID.isEqual(mockConfigurationAttributeElemID))
    ...
    describe('Field lookup with valuesMapping defined', () => {
      it('Should change lookup field value set fullName value to a ref if custom field exists (mapped and not) and keep string if not', () => {
        const priceScheduling = elements.find(e => e.elemID.isEqual(mockPriceScheduleElemID))
    ...
    describe('Modification changes', () => {
      ...
      describe('Object lookup', () => {
        it('Should not change in object modifications', () => {
          ...
          expect(afterData.fields[CPQ_DEFAULT_OBJECT_FIELD].annotations.valueSet[0].fullName).toEqual(
            mockAfterResolveConfigurationAttribute.fields[CPQ_DEFAULT_OBJECT_FIELD].annotations.valueSet[0].fullName,
          )
          expect(afterData.fields[CPQ_DEFAULT_OBJECT_FIELD].annotations.valueSet[1].fullName).toEqual(
            mockAfterResolveConfigurationAttribute.fields[CPQ_DEFAULT_OBJECT_FIELD].annotations.valueSet[1].fullName,
          )
          expect(afterData.fields[CPQ_DEFAULT_OBJECT_FIELD].annotations.valueSet[2].fullName).toEqual(
            mockAfterResolveConfigurationAttribute.fields[CPQ_DEFAULT_OBJECT_FIELD].annotations.valueSet[2].fullName,
          )
        })
        it('Should in fields modifications map back to the service name instead of api if theres mapping and keep same if not', () => {
          const configAttrFieldChange = changes.find(change =>
            getChangeData(change).elemID.isEqual(mockConfigurationAttribute.fields[CPQ_DEFAULT_OBJECT_FIELD].elemID),
          )
          expect(configAttrFieldChange).toBeDefined()
          const afterData = (configAttrFieldChange as ModificationChange<Field>).data.after
          expect(afterData.annotations.valueSet[0].fullName).toEqual(CPQ_QUOTE_NO_PRE)
          expect(afterData.annotations.valueSet[1].fullName).toEqual(existingObjectName)
          expect(afterData.annotations.valueSet[2].fullName).toEqual('nonExistingObject')
        })
      })
      describe('Field lookup', () => {
        it('Should not change in object modifications', () => {
          ...
          expect(afterData.fields[CPQ_CONSTRAINT_FIELD].annotations.valueSet[0].fullName).toEqual(
            mockAfterResolvePriceSchedule.fields[CPQ_CONSTRAINT_FIELD].annotations.valueSet[0].fullName,
          )
          expect(afterData.fields[CPQ_CONSTRAINT_FIELD].annotations.valueSet[1].fullName).toEqual(
            mockAfterResolvePriceSchedule.fields[CPQ_CONSTRAINT_FIELD].annotations.valueSet[1].fullName,
          )
          expect(afterData.fields[CPQ_CONSTRAINT_FIELD].annotations.valueSet[2].fullName).toEqual(
            mockAfterResolvePriceSchedule.fields[CPQ_CONSTRAINT_FIELD].annotations.valueSet[2].fullName,
          )
        })
        it('Should in field modification map back to the service name instead of api if theres mapping and keep same if not', () => {
          const priceSchedulingFieldChange = changes.find(change =>
            getChangeData(change).elemID.isEqual(mockPriceSchedule.fields[CPQ_CONSTRAINT_FIELD].elemID),
          )
          ...
          expect(afterData.annotations.valueSet[0].fullName).toEqual(CPQ_ACCOUNT_NO_PRE)
          expect(afterData.annotations.valueSet[1].fullName).toEqual(existingFieldName)
          expect(afterData.annotations.valueSet[2].fullName).toEqual('nonExistingField')
        })
      })
    })
    describe('Addition changes', () => {
      ...
      describe('Object lookup', () => {
        it('Should map back to the service name instead of api if theres mapping and keep same if not', () => {
          ...
          expect(afterData.fields[CPQ_DEFAULT_OBJECT_FIELD].annotations.valueSet[0].fullName).toEqual(CPQ_QUOTE_NO_PRE)
          expect(afterData.fields[CPQ_DEFAULT_OBJECT_FIELD].annotations.valueSet[1].fullName).toEqual(
            existingObjectName,
          )
          expect(afterData.fields[CPQ_DEFAULT_OBJECT_FIELD].annotations.valueSet[2].fullName).toEqual(
      ...
      describe('Field lookup', () => {
        it('Should map back to the service name instead of api if theres mapping and keep same if not', () => {
          ...
          expect(afterData.fields[CPQ_CONSTRAINT_FIELD].annotations.valueSet[0].fullName).toEqual(CPQ_ACCOUNT_NO_PRE)
          expect(afterData.fields[CPQ_CONSTRAINT_FIELD].annotations.valueSet[1].fullName).toEqual(existingFieldName)
          expect(afterData.fields[CPQ_CONSTRAINT_FIELD].annotations.valueSet[2].fullName).toEqual('nonExistingField')
        })
      })
    })
  })
    ...
    describe('Modification changes', () => {
      ...
      describe('Object lookup', () => {
        it('Should not change on object modifications', () => {
          ...
          const afterData = (configAttrChange as ModificationChange<ObjectType>).data.after
          expect(afterData.fields[CPQ_DEFAULT_OBJECT_FIELD].annotations.valueSet[0].fullName).toEqual(
            mockConfigurationAttribute.fields[CPQ_DEFAULT_OBJECT_FIELD].annotations.valueSet[0].fullName,
          )
          expect(afterData.fields[CPQ_DEFAULT_OBJECT_FIELD].annotations.valueSet[1].fullName).toEqual(
            mockConfigurationAttribute.fields[CPQ_DEFAULT_OBJECT_FIELD].annotations.valueSet[1].fullName,
          )
          expect(afterData.fields[CPQ_DEFAULT_OBJECT_FIELD].annotations.valueSet[2].fullName).toEqual(
            mockConfigurationAttribute.fields[CPQ_DEFAULT_OBJECT_FIELD].annotations.valueSet[2].fullName,
          )
        })
      ...
      describe('Field lookup', () => {
        ...
        it('Should on field modification map to api name if theres mapping and keep same if not', () => {
          ...
          expect(afterData.annotations.valueSet[0].fullName).toEqual('SBQQ__Quote__c.SBQQ__Account__c')
          expect(afterData.annotations.valueSet[1].fullName).toEqual(`SBQQ__Quote__c.${existingFieldName}`)
          // This is a known issue
          expect(afterData.annotations.valueSet[2].fullName).toEqual('SBQQ__Quote__c.nonExistingField')
        })
      })
    })
    describe('Addition changes', () => {
      ...
      describe('Object lookup', () => {
        it('Should map to api name if theres mapping and keep same if not', () => {
          ...
          expect(configAttrChange).toBeDefined()
          const afterData = (configAttrChange as ModificationChange<ObjectType>).data.after
          expect(afterData.fields[CPQ_DEFAULT_OBJECT_FIELD].annotations.valueSet[0].fullName).toEqual(CPQ_QUOTE)
          expect(afterData.fields[CPQ_DEFAULT_OBJECT_FIELD].annotations.valueSet[1].fullName).toEqual(
            existingObjectName,
          )
          expect(afterData.fields[CPQ_DEFAULT_OBJECT_FIELD].annotations.valueSet[2].fullName).toEqual(
