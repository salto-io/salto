...
import {
  ...
  INSTANCE_ANNOTATIONS, isInstanceElement, Field,
...
describe('WorklowFieldUpdate filter', () => {
  ...
  const generateObjectAndInstance = ({
    ...
    const obj = new ObjectType({
      ...
      fields: {
        ...
        ignore: { type: BuiltinTypes.NUMBER },
      },
    })
    ...
    if (objType === CUSTOM_OBJECT || fieldValue === undefined) {
      const customObj = new InstanceElement(type,
        customObjectType,
        {
          [CUSTOM_OBJECT_ID_FIELD]: type,
        })
      return [obj, customObj]
    }
    ...
    const instance = new InstanceElement(
      ...
      {
        ...(parentType
          ? { [INSTANCE_ANNOTATIONS.PARENT]: new ElemID(SALESFORCE, parentType) }
          : {}),
      },
    )
    return [obj, instance]
  ...
  const generateElements = (
    ...
    // filterItem643.field should point to Account.name (included)
    ...generateObjectAndInstance({
      type: 'FilterItem',
      objType: 'FilterItem',
  ...
  describe('on fetch', () => {
    ...
    it('should resolve custom object instances', () => {
      ...
      expect(inst.value.reportType?.elemId.getFullName()).toEqual(elements[2].elemID.getFullName())
    })
    ...
    // TODO add check: fail to resolve because resolving back returns a different value
  })
})
