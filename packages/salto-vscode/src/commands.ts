...
const shouldApply = async (planActions: plan.Plan, extensionPath: string): Promise<boolean> => {
...
export const planCommand = async (
  ...
  if (!workspace.hasErrors()) {
    displayPlan(await api.plan(workspace.workspace), extensionPath)
  } else {
...
export const applyCommand = async (
  ...
  const initApplyProgress = async (
    processPromise: Promise<plan.Plan>,
  ...
  let applyProcess: Promise<plan.Plan>
  const shouldApplyCB = async (p: plan.Plan): Promise<boolean> => shouldApply(p, extensionPath)
  ...
  const updateActionCB = async (action: plan.PlanItem): Promise<void> => {
    if (!progress) {
