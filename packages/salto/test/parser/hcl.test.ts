...
describe('HCL Parser', () => {
  ...
  const devalute = (value: any): Record<string, any> => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const devaluteValue = (v: any): Record<string, any> => ({
      type: 'literal',
      value: v,
    })

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const devaluteString = (str: string): Record<string, any> => ({
      type: 'template',
      expressions: [devaluteValue(str)],
    })

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const devaluteArray = (arr: any[]): Record<string, any> => ({
      type: 'list',
      expressions: arr.map(e => devalute(e)),
    })

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const devaluteObject = (obj: Record<string, any>): Record<string, any> => ({
      type: 'map',
