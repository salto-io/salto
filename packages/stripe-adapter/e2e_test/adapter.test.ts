...
// assumes the mapping: v1__<plural-typeName> -> <plural-typeName> is declared for all the supported types.
describe('Stripe adapter E2E with real swagger and mock replies', () => {
    let swaggerObjectTypes: ObjectType[]
    ...
    beforeAll(async () => {
        ...
        swaggerObjectTypes = elements.filter(isObjectType)
    })
    ...
    describe('fetches swagger types', () => {
        test.each(TYPE_NAMES)(
            '%s',
            async typeName => {
                expect(swaggerObjectTypes.some(e => e.elemID.getFullName() === typeName))
            })
    })
})