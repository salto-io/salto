...
describe('Field references filter', () => {
  ...
  describe('on fetch', () => {
    ...
    it('should resolve field values when referenced element exists', () => {
      const price = elements.filter(
        e => isInstanceElement(e) && e.elemID.name === 'price'
      ...
      const plan = elements.filter(
        e => isInstanceElement(e) && e.elemID.name === 'plan'
      )[0] as InstanceElement
