...
const adapterConfigFromConfig = (config: Readonly<InstanceElement> | undefined): WorkatoConfig => {
  ...
  const defaultConfig = getDefaultConfig(configValue.fetch?.enableExperimentalSupportingDeploy)
...
export const adapter: Adapter = {
  operations: context => {
    ...
    const updatedConfig = configUtils.configMigrations.migrateDeprecatedIncludeList(
      ...
      getDefaultConfig(context.config?.value.fetch?.enableExperimentalSupportingDeploy),
    )
