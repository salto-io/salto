...
export const getDefaultConfig = (deploySupported = false, deployWithReferencesSupported = false): WorkatoConfig => {
  ...
  if (deployWithReferencesSupported) {
    typeDefaultTransformation.fieldsToOmit = typeDefaultTransformation.fieldsToOmit !== undefined ? [...typeDefaultTransformation.fieldsToOmit, ...FIELDS_TO_OMIT_WHILE_SUPPORT_DEPLOY_WITH_REFERENCES] : FIELDS_TO_OMIT_WHILE_SUPPORT_DEPLOY_WITH_REFERENCES
