...
export const EXPERIMENTAL_SUPPORTED_DEPLOY_FIELDS_TO_OMIT: configUtils.FieldToOmitType[] = [
  { fieldName: 'created_at', fieldType: 'string' },
  { fieldName: 'updated_at', fieldType: 'string' },
...
export type WorkatoFetchConfig = definitions.UserFetchConfig & {
  ...
  enableExperimentalSupportingDeploy?: boolean
}
...
export const getDefaultConfig = (experimentalSupportingDeploy = false): WorkatoConfig => {
  if (experimentalSupportingDeploy) {
    const defaultConfig = _.cloneDeep(DEFAULT_CONFIG)
    defaultConfig[FETCH_CONFIG].enableExperimentalSupportingDeploy = true
    if (defaultConfig[API_DEFINITIONS_CONFIG].typeDefaults.transformation !== undefined) {
      defaultConfig[API_DEFINITIONS_CONFIG].typeDefaults.transformation.fieldsToOmit =
        EXPERIMENTAL_SUPPORTED_DEPLOY_FIELDS_TO_OMIT
    }
    if (defaultConfig[API_DEFINITIONS_CONFIG].types !== undefined) {
      defaultConfig[API_DEFINITIONS_CONFIG].types = getDefaultTypes(EXPERIMENTAL_SUPPORTED_DEPLOY_FIELDS_TO_OMIT)
    }
    ...
    [FETCH_CONFIG]: {
      refType: definitions.createUserFetchConfigType({
        ...
        additionalFields: {
          ...
          enableExperimentalSupportingDeploy: { refType: BuiltinTypes.BOOLEAN },
        },
        omitElemID: true,
      }),
    },
