...
const isJiraFieldMatchGroup = (val: Values): val is JiraFieldMatchGroup => (
...
export const addJiraRecipeReferences = async (
  ...
  const formulaReferenceFinder: FormulaReferenceFinder = (value, path) => {
    ...
    return potentialFields.map((fieldNameId: string): MappedReference | undefined => {
