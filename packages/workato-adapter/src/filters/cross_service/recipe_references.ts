...
const getServiceConnectionDetails = (
  ...
  const connectionInstanceDetails = Object.fromEntries(
    connectionInstances
      ...
      .map(inst => [inst.value.name as string, { id: inst.elemID, applicationName: inst.value.application as string }]),
  )
  const connectionDetailsByService = _.mapValues(serviceConnectionConfig, connectionNames =>
    Object.fromEntries(
  ...
  const missingConnections = Object.entries(serviceConnectionConfig).flatMap(([serviceName, connectionNames]) =>
    makeArray(connectionNames).filter(name => connectionDetailsByService[serviceName][name] === undefined),
  )
...
const filterRelevantRecipeCodes = (
  ...
  const relevantRecipes = recipeInstances
    .filter(recipe => isReferenceExpression(recipe.value.code))
    .filter(
      recipe =>
        Array.isArray(recipe.value.config) &&
        recipe.value.config.some(
          connectionConfig =>
  ...
  return relevantRecipes.map(recipe => recipeCodeInstances[recipe.value.code.elemID.getFullName()])
}
...
const addReferencesForConnectionRecipes = async (
  ...
  if (serviceName === SALESFORCE) {
    ...
    await awu(relevantRecipeCodes).forEach(inst => addSalesforceRecipeReferences(inst, index, appName))
    return
  }
  if (serviceName === NETSUITE) {
    const index = indexNetsuiteByTypeAndScriptId(serviceElements)
    await awu(relevantRecipeCodes).forEach(inst => addNetsuiteRecipeReferences(inst, index, appName))
  }

  if (serviceName === ZUORA_BILLING) {
    const index = indexZuoraByElemId(serviceElements)
    await awu(relevantRecipeCodes).forEach(inst => addZuoraRecipeReferences(inst, index, appName))
  }

  if (serviceName === JIRA) {
    const index = indexJira(serviceElements)
    await awu(relevantRecipeCodes).forEach(inst => addJiraRecipeReferences(inst, index, appName))
  }

  if (serviceName === ZENDESK) {
    const index = indexZendesk(serviceElements)
    await awu(relevantRecipeCodes).forEach(inst => addZendeskRecipeReferences(inst, index, appName))
  }
}
  ...
  onPostFetch: async ({
    ...
    if (serviceConnectionNames !== undefined) {
      ...
      if (unsupportedAccounts.length > 0) {
        log.error(
          `The following account names don't exist in the workspace: ${unsupportedAccounts}. Existing accounts are ${supportedAdapters}.`,
        )
        return
      }
    }
    const serviceConnectionDetails = getServiceConnectionDetails(
      ...
      currentAdapterElements.filter(isInstanceElement).filter(inst => inst.refType.elemID.name === 'connection'),
    )
    const recipeInstances = currentAdapterElements
    ...
    await awu(Object.entries(serviceConnectionDetails)).forEach(async ([accountName, connections]) => {
      await awu(Object.values(connections)).forEach(async ({ id, applicationName }) => {
        const relevantRecipeCodes = filterRelevantRecipeCodes(id, recipeInstances, recipeCodeInstancesByElemID)
