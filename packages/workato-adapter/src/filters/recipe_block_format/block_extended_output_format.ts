...
const isExtendedSchemaItem = createSchemeGuard<ExtendedSchemaItem>(EXTENDED_SCHEMA_ITEM_SCHEMA)
...
const getTransformToMinimalExtendedOutputSchemaFunc = (formulasByBlock: Record<string, Values>): TransformFunc => {
  ...
  return async ({ value, path }) => {
    if (path !== undefined) {
      ...
      else if (endPathPart === 'extended_output_schema' && _.isArray(value)) {
...
const filterCreator: FilterCreator = ({ config }) => ({
  ...
  onFetch: async (elements: Element[]) => {
    if (config.enableDeployWithReferencesSupport === false) {
      log.debug('Parsing recipe block extended output schema format was disabled')
      return
    }
    await Promise.all(
      elements
