...
const getApiCollectionUrl: GetUrlFunc = instance =>
  instance.value.id && `${BASE_URL}/api_groups/${instance.value.id}/endpoints`
...
const getApiEndpointUrl: GetUrlFunc = instance => {
  const base = instance.value.api_collection_id?.value && getApiCollectionUrl(instance.value.api_collection_id.value)
...
const filter: FilterCreator = () => ({
  ...
  onFetch: async (elements: Element[]) => {
    elements
      ...
      .forEach(element => {
        const url = ID_TO_URL_GENERATOR[element.elemID.typeName](element)
