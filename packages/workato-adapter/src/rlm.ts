...
const getFullPath = (elem: InstanceElement): string => path.join(
  ...elem.value.folder_id.folderParts, `${halfSnakeCase(elem.value.name)}.${elem.elemID.typeName}.json`
)

const isIdResponse = createSchemeGuard<{ id: number }>(Joi.object({
  id: Joi.number().required(),
}).unknown(true).required(), 'Received an invalid project id response')

const isStatusResponse = createSchemeGuard<{ status: string }>(Joi.object({
  status: Joi.string().required(),
}).unknown(true).required(), 'Received an invalid project status response')

const isErrorResponse = createSchemeGuard<{ error: string }>(Joi.object({
  error: Joi.string().required(),
}).unknown(true).required(), 'Received an invalid project error response')

const isMessageResponse = createSchemeGuard<{ message: string }>(Joi.object({
  message: Joi.string().required(),
}).unknown(true).required(), 'Received an invalid project message response')
...
export const isConnection = createSchemeGuardForInstance<Connection & InstanceElement>(
  CONNECTION_SCHEMA.required(), 'Received an invalid value for connection'
)

export const isRecipe = createSchemeGuardForInstance<JsonRecipe & InstanceElement>(
  RECIPE_SCHEMA.required(), 'Received an invalid value for recipe'
)
/**
...
const checkRLMImportStatus = async (client: WorkatoClient, jobId: number): Promise<boolean> => {
  const res = await client.getSinglePage({ url: `/packages/${jobId}` })
...
const pollRLMImportStatus = async (client: WorkatoClient, jobId: number):
Promise<void> => {
...
const getWorkatoError = (elemList: ElemID[], error: Error): Error => {
  ...
  if ([400, 401, 404, 500].includes(error.response.status)) {
    const errorResponse = [baseErrorMessage, `status: ${error.response.status}`, `message: ${error.response.statusText}`]
...
const recipeToZipFormat = async (zip: JSZip, recipe: InstanceElement) : Promise<void> => {
  const configList = recipe.value.config.map((conf: RecipeConfig) => ({
    ...
    // eslint-disable-next-line camelcase
    account_id: conf.account_id !== undefined ? {
      zip_name: path.join(...conf.account_id.folder_id.folderParts, `${halfSnakeCase(conf.account_id.name)}.connection.json`),
      name: conf.account_id.name,
      folder: conf.account_id.folder_id.folderParts.length !== 0 ? path.join(...conf.account_id.folder_id.folderParts) : '',
    } : null,
  }))
...
const connectionToZipFormat = (zip: JSZip, connection: InstanceElement) : void => {
  zip.file(getFullPath(connection), JSON.stringify({
...
const makeRLMZipFormat = (changes: Change<InstanceElement>[]) : [JSZip, DeployResult] => {
  ...
  const [validConnections, invalidConnections] = _.partition(
    connectionChanges, connection => isConnection(getChangeData(connection))
  )
  const [validRecipes, invalidRecipes] = _.partition(
    Array.from(new Set(recipeChanges)), recipe => isRecipe(getChangeData(recipe))
  )
...
const RLMImportZip = async ({ zip, client, rootId, elemList }: {
  zip: JSZip
