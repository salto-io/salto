...
const extractPageEntries: clientUtils.PageEntriesExtractor = page => makeArray(page) as clientUtils.ResponseValue[]
...
describe('client_pagination', () => {
  describe('getMinSinceIdPagination', () => {
    ...
    it('should query a single page if paginationField is not specified', async () => {
      ...
      const result = (
        await toArrayAsync(clientUtils.traverseRequests(getMinSinceIdPagination(args), extractPageEntries)(args))
      ).flat()
