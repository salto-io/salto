...
describe('blockInputFilter filter', () => {
  ...
  beforeAll(() => {
    ...
    filter = filterCreator({
      ...
      config: getDefaultConfig(true, true),
  ...
  describe('onFetch', () => {
    ...
    it('should build new input from valid input', async () => {
      const validNewInput = [
        {
          key: 'test1',
          value: 'test1_value',
      ...
      // valid block
      expect(recipeCodeServerSide.value.block[0].block[0].input).toEqual(validNewInput)
      // with invalid input keys block
      expect(recipeCodeServerSide.value.block[1].input).toEqual(withInvalidNewInput)
    })
  })
})
