...
const ctorNameToSerializedName: Record<string, SerializedName> = _(NameToType)
  .entries()
  .map(([name, type]) => [type.name, name])
  .fromPairs()
  .value()
...
export const serializeStream = async <T = Element>(
  ...
  const staticFileReplacer = (
    e: StaticFile,
  ): Omit<types.PickDataFields<StaticFile> & SerializedClass, 'internalContent' | 'content'> => {
...
const generalDeserializeParsed = async <T>(parsed: unknown, staticFileReviver?: StaticFileReviver): Promise<T[]> => {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const staticFiles: { obj: any; key: string | number }[] = []
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const restoreClasses = (value: any): any => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const reviveElemID = (v: Record<string, any>): ElemID => new ElemID(v.adapter, v.typeName, v.idType, ...v.nameParts)
    ...
    const revivers: ReviverMap = {
      ...
      Variable: v => new Variable(reviveElemID(v.elemID), restoreClasses(v.value)),
      PrimitiveType: v =>
        new PrimitiveType({
          elemID: reviveElemID(v.elemID),
      ...
      InvalidValueTypeValidationError: v =>
      ...
      InvalidTypeValidationError: v => new InvalidTypeValidationError(reviveElemID(v.elemID)),
    }
...
const generalDeserialize = async <T>(data: string, staticFileReviver?: StaticFileReviver): Promise<T[]> => {
...
export const deserializeValues = async (data: string, staticFileReviver?: StaticFileReviver): Promise<Value> =>
  generalDeserialize(data, staticFileReviver)

export const deserialize = async (data: string, staticFileReviver?: StaticFileReviver): Promise<Element[]> => {
...
export const deserializeParsed = async (parsed: unknown, staticFileReviver?: StaticFileReviver): Promise<Element[]> => {
