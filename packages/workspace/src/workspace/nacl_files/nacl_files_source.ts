...
const buildNaclFilesState = async ({
  ...
  const getNaclFileAndCheckValidity = async (file: ParsedNaclFile):
    ...
    const shouldInvalidateCache = cacheValid
    ...
    cacheValid = shouldInvalidateCache && (res !== undefined)
    return res
  }
...
const buildNaclFilesSource = (
  ...
  const getParsedNaclFile = async (filename: string): Promise<ParsedNaclFile | undefined> => {
    ...
    return (await parseNaclFiles([naclFile], (await getState()).parsedNaclFiles, functions))[0]
  }
