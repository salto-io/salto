...
const updateIndex = async (
  ...
  const uniqueModifications = modifications.map(modification => ({
    ...modification,
    value: _.uniqBy(modification.value, id => id.getFullName()),
  }))
  await Promise.all([
    index.setAll(uniqueModifications),
    index.deleteAll(deletions.map(deletion => deletion.key)),
  ])
}
...
export const updateReferenceIndexes = async (
  ...
  await Promise.all([
    updateReferenceTargetsIndex(
      relevantChanges,
      referenceTargetsIndex,
      changeToReferences,
    ),
    updateReferenceSourcesIndex(
      relevantChanges,
