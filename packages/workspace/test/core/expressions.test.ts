...
import { ElemID, ObjectType, BuiltinTypes, InstanceElement, Element, ReferenceExpression, VariableExpression, TemplateExpression, ListType, Variable, isVariableExpression, isReferenceExpression, StaticFile, PrimitiveType, PrimitiveTypes, ReferenceType } from '@salto-io/adapter-api'
...
describe('Test Salto Expressions', () => {
  ...
  describe('Reference Expression', () => {
    ...
    it('should resolve the top level element in a reference when resolveRoot is true', async () => {
      ...
      const refTargetInst = new InstanceElement('rrr', new ReferenceType(refTargetInstObj.elemID), {
        test: 'okok',
      })
      const instanceToResolve = new InstanceElement('rrr', new ReferenceType(instanceToResolveObj.elemID), {
