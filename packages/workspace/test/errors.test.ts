...
import { UnresolvedReferenceGroupError, groupUnresolvedRefsByTarget } from '../src/errors'
import { UnresolvedReferenceValidationError } from '../src/validator'

describe('groupUnresolvedRefsByTarget', () => {
  ...
  describe('group unresolved references by target', () => {
    ...
    describe('with references to the same target', () => {
      ...
      beforeEach(() => {
        ...
        originalErrors = [
          nonGroupedError,
          ...sources.map(
            elemID => new UnresolvedReferenceValidationError({ elemID, target })
          ),
        ]
        groupedErrors = groupUnresolvedRefsByTarget(originalErrors)
      })
      it('should group related errors', () => {
        expect(groupedErrors.length).toBeLessThan(originalErrors.length)
      })
      it('should disregard non unresolved errors', () => {
        expect(groupedErrors).not.toContain(nonGroupedError)
      })
    ...
    describe('with references to targets nested in other targets', () => {
      ...
      beforeEach(() => {
        ...
        originalErrors = sources.map(
          (elemID, idx) => new UnresolvedReferenceValidationError(
    ...
    describe('with a single unresolved reference', () => {
      ...
      beforeEach(() => {
        target = new ElemID('test', 'target')
        originalErrors = [
          new UnresolvedReferenceValidationError({ elemID: sources[0], target }),
        ]
        groupedErrors = groupUnresolvedRefsByTarget(originalErrors)
      })
