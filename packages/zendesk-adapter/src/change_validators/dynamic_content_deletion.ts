...
export const dynamicContentDeletionValidator: ChangeValidator = async (changes, elementSource) => {
  ...
  const dynamicContentRemovals = changes.filter(isInstanceChange).filter(isRemovalChange).map(getChangeData)
  ...
  const errors = Object.values(dynamicContentToUsages).map(({ elemId, usages }): ChangeError | undefined => {
    ...
    return {
      ...
      detailedMessage: `This dynamic content is being used and cannot be deleted, used by: ${_.uniq(usages).join(', ')}`,
    }
  }).filter(isDefined)
