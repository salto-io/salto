...
const validateNoDuplicateChild = ({
  orderInstances,
  orderField,
}: {
  ...
  orderInstances.forEach(orderInstance => {
    const children = orderInstance.value[orderField]
...
const validateOrderElementAdded = ({
  ...
  const childrenInOrderInstances = new Set(
    orderInstances
  ...
  return childInstances
    .filter(child => !childrenInOrderInstances.has(child.elemID.getFullName()))
    .map(child => notInOrderError({ instance: child, orderTypeName, defaultLocation: 'beginning' }))
}

const validateChildrenInOrder = ({
  changes,
  orderField,
  orderTypeName,
  childTypeName,
  ...
  const relevantOrderInstances = changes
    ...
    .map(getChangeData)
    .filter(order => order.elemID.typeName === orderTypeName)
    .filter(order => validateOrderType(order, orderField))

  const childInstances = changes
    .filter(isInstanceChange)
    .filter(isAdditionChange)
    .map(getChangeData)
    .filter(child => child.elemID.typeName === childTypeName)
