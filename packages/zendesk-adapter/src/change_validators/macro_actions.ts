...
import {
  ...
  isInstanceElement, isReferenceExpression, ReadOnlyElementsSource, ReferenceExpression,
 ...
 */
const deactivatedTicketFieldInActions = async (macro: InstanceElement, elementSource: ReadOnlyElementsSource)
  : Promise<string[]> => {
  const actionWithTicketFields: ActionsType[] = macro.value.actions
    ?.filter(isAction)
    .filter((action: ActionsType) =>
      isReferenceExpression(action.field) && action.field.elemID.typeName === 'ticket_field')
...
export const macroActionsTicketFieldDeactivationValidator: ChangeValidator = async (
  ...
  return macroWithDeactivatedTicketFieldData
