...
import {
  ChangeError,
  FixElementsFunc, GetCustomReferencesFunc, InstanceElement, ReadOnlyElementsSource,
  ReferenceExpression,
  ReferenceInfo, isInstanceElement, isReferenceExpression,
...
const getFixedElementsAndUpdatedPaths = (elementsSource: ReadOnlyElementsSource) => async (instance: InstanceElement):
  ...
  const changedPaths = instanceOrderPaths.filter(path =>
    _.get(fixedInstance.value, path)?.length !== _.get(instance.value, path)?.length)
 ...
 */
const removeMissingOrderElements:
  WeakReferencesHandler<{
    elementsSource: ReadOnlyElementsSource
  }>['removeWeakReferences'] = ({ elementsSource }): FixElementsFunc => async elements => {
    ...
    const errors: ChangeError[] = fixedElementsWithPaths.flatMap(({ instance, paths }) => paths.map(path => {
