...
import {
  ...
  isInstanceChange, isRemovalChange, isReferenceExpression,
  ...
  const relevantInstanceChanges = Array.from(changes.entries())
    ...
    .filter(
      (change): change is deployment.dependency.ChangeWithKey<Change<InstanceElement>> =>
        isInstanceChange(change.change) && isRemovalChange(change.change)
      && [CUSTOM_OBJECT_TYPE_NAME, CUSTOM_OBJECT_FIELD_TYPE_NAME].includes(getChangeData(change.change).elemID.typeName)
    )

  const customObjectFieldRemovals = relevantInstanceChanges
    .filter(({ change }) => getChangeData(change).elemID.typeName === CUSTOM_OBJECT_FIELD_TYPE_NAME)
  const customObjectRemovalsByName = _.keyBy(
    relevantInstanceChanges
      .filter(({ change }) => getChangeData(change).elemID.typeName === CUSTOM_OBJECT_TYPE_NAME),
    ({ change }) => getChangeData(change).elemID.name
  )

  return customObjectFieldRemovals.flatMap(({ key, change }) => {
