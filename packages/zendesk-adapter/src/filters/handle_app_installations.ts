...
const valueToTemplate = (
  val: string,
  regex: RegExp,
  locatorTypes: string[],
  typeToElements: Record<string, InstanceElement[]>,
...
const runFunctionOnLocatedFields = (
  app: InstanceElement,
  locators: IdLocator[],
  func: (field: string, locator: IdLocator) => void,
): void =>
...
const replaceFieldsWithTemplates = (
  ...
  const convertValuesToTemplate = (field: string, locator: IdLocator): void => {
    if (app.value.settings && _.isString(app.value.settings[field])) {
      app.value.settings[field] = valueToTemplate(
        app.value.settings[field],
        new RegExp(locator.idRegex, 'gi'),
        locator.type,
        typeToElements,
      )
    }
...
const filterCreator: FilterCreator = ({ config }) => {
  ...
  const locators = fetchConfig.greedyAppReferences ? [ALL_LOCATOR] : fetchConfig.appReferenceLocators ?? LOCATORS
  return {
    name: 'handleAppInstallationsFilter',
    onFetch: async (elements: InstanceElement[]): Promise<void> =>
      getAppInstallations(elements).forEach(app =>
        replaceFieldsWithTemplates(
          app,
          _.groupBy(
            elements.filter(e =>
              [
                TICKET_FIELD_TYPE_NAME, // before it was the values of ZENDESK_REFERENCE_TYPE_TO_SALTO_TYPE
    ...
    preDeploy: async (changes: Change<InstanceElement>[]): Promise<void> =>
      getAppInstallations(changes.map(getChangeData)).forEach(app => {
        const convertTemplatesToValues = (fieldName: string): void => {
          replaceTemplatesWithValues(
            { fieldName, values: [app.value.settings] },
