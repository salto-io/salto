...
import {
  ValueReplacer,
  deployModificationFunc,
  replaceConditionsAndActionsCreator,
  fieldReplacer,
} from '../replacers_utils'
...
const isOrganizationsResponse = createSchemeGuard<OrganizationResponse>(
  EXPECTED_ORGANIZATION_RESPONSE_SCHEMA,
  'Received an invalid response from organization request',
)
const isSingleOrganizationResponse = createSchemeGuard<SingleOrganizationResponse>(
  EXPECTED_SINGLE_ORGANIZATION_RESPONSE_SCHEMA,
  'Received an invalid response from single organization request',
)
    ...
    await Promise.all(
      organizationNames.map(async organizationName => {
        ...
        const res = (
          ...
          .flat()
          .flatMap(response => response.organizations)
        ...
        if (organization === undefined) {
          log.debug(
            `could not find any organization with name ${organizationName}, if createMissingOrganizations it will be created`,
          )
...
export const createOrganizationPathEntries = (instances: InstanceElement[]): organizationIdInstanceAndPath[] => {
  const organizationPathsEntries = instances
    .flatMap(instance => {
      const organizationPaths = TYPE_NAME_TO_REPLACER[instance.elemID.typeName]?.(instance) ?? []
      return organizationPaths.map(path => {
...
const filterCreator: FilterCreator = ({ client, config }) => {
  ...
  return {
    ...
    onFetch: async elements => {
      ...
      const mapping = Object.fromEntries(organizations.map(org => [org.id.toString(), org.name]))
    ...
    preDeploy: async (changes: Change<InstanceElement>[]) => {
      ...
      const organizationNames = _.uniq(
        relevantChanges.flatMap(change => {
      ...
      organizationIdToName = Object.fromEntries(organizations.map(org => [org.id.toString(), org.name])) as Record<
        string,
        string
      >
