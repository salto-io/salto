...
describe('automationAllConditionsValidator', () => {
  ...
  const notValidAutomation = new InstanceElement('Test2', automationType, {
    id: 2,
    title: 'Test',
    active: true,
    actions: [
  ...
  const noConditionInAutomation = new InstanceElement('Test3', automationType, {
    id: 2,
    title: 'Test',
    active: true,
    actions: [
  ...
  const noAllInConditionAutomation = new InstanceElement('Test4', automationType, {
    id: 2,
    title: 'Test',
    active: true,
    actions: [
  ...
  const notValidAutomationSeveralConditions = new InstanceElement('Test5', automationType, {
    id: 2,
    title: 'Test',
    active: true,
    actions: [
  ...
  const validAutomationSeveralConditions = new InstanceElement('Test6', automationType, {
    id: 2,
    title: 'Test',
    active: true,
    actions: [
  ...
  it('should return an error when automation does not contain necessary field condition', async () => {
    const errors = await automationAllConditionsValidator([toChange({ after: notValidAutomation })])
  ...
  it('should return an error when automation does not contain necessary field condition of several conditions', async () => {
    const errors = await automationAllConditionsValidator([toChange({ after: notValidAutomationSeveralConditions })])
    expect(errors).toEqual([
      {
  ...
  it('should not return an error when automation contains necessary field condition', async () => {
    const errors = await automationAllConditionsValidator([toChange({ after: validAutomation })])
    expect(errors).toHaveLength(0)
  })
  it('should not return an error when automation contains necessary field condition out of several conditions', async () => {
    const errors = await automationAllConditionsValidator([toChange({ after: validAutomationSeveralConditions })])
    expect(errors).toHaveLength(0)
  })
  it('should not return an error when we remove an item', async () => {
    const errors = await automationAllConditionsValidator([toChange({ before: validAutomation })])
    expect(errors).toHaveLength(0)
  })
  it('should return error when there are no conditions', async () => {
    const errors = await automationAllConditionsValidator([toChange({ after: noConditionInAutomation })])
    expect(errors).toEqual([
      {
  ...
  it('should return error when there are no All conditions', async () => {
    const errors = await automationAllConditionsValidator([toChange({ after: noAllInConditionAutomation })])
    expect(errors).toEqual([
      {
  ...
  it('should not return error for reference expression', async () => {
    ...
    const ticketFieldInstance = new InstanceElement('field_test', ticketFieldType, {
      id: 123,
    })
    const automationWithReference = replaceInstanceTypeForDeploy({
      instance: new InstanceElement('Test10', automationType, {
        id: 2,
        title: 'Test10',
        active: true,
        actions: [
    ...
    const errors = await automationAllConditionsValidator([toChange({ after: automationWithReference })])
    expect(errors).toHaveLength(0)
  })
})
