...
const createUserField = (name: string): InstanceElement => new InstanceElement(
  name,
  new ObjectType({ elemID: new ElemID(ZENDESK, USER_FIELD_TYPE_NAME) }),
  {
    key: 'test_key',
    type: 'text',
  },
...
describe('immutableTypeAndKeyForUserFieldsValidator', () => {
  ...
  it('should return an error when the user field key is changed', async () => {
    ...
    const changes = [
      toChange({ before: userFieldInstance, after: changedUserFieldInstance }),
    ]
