...
describe('oneTranslationPerLocalValidator', () => {
  ...
  it('should not return an error when article has different translations with different locale', async () => {
    ...
    const esLocale = new InstanceElement('es', guideLanguageSettingsType, {
      locale: 'es',
    })
    const article = new InstanceElement('Test1', articleType, {
      translations: [
    ...
    heTranslation.annotations[CORE_ANNOTATIONS.PARENT] = [new ReferenceExpression(article.elemID, article)]
    enTranslation.annotations[CORE_ANNOTATIONS.PARENT] = [new ReferenceExpression(article.elemID, article)]
    esTranslation.annotations[CORE_ANNOTATIONS.PARENT] = [new ReferenceExpression(article.elemID, article)]
  ...
  it('should return an error when article has different translations with same locale', async () => {
    const enusLocale = new InstanceElement('en-us', guideLanguageSettingsType, {
      locale: 'en-us',
    })
    const enTranslation = new InstanceElement(
      'Test2',
    ...
    const article = new InstanceElement('Test2', articleType, {
      translations: [
        new ReferenceExpression(articleTranslationType.elemID.createNestedID('instance', 'Test2'), enTranslation2),
        new ReferenceExpression(articleTranslationType.elemID.createNestedID('instance', 'Test2'), enTranslation),
      ],
    })
    enTranslation2.annotations[CORE_ANNOTATIONS.PARENT] = [
      new ReferenceExpression(articleType.elemID.createNestedID('instance', 'Test2')),
    ]
    enTranslation.annotations[CORE_ANNOTATIONS.PARENT] = [
      new ReferenceExpression(articleType.elemID.createNestedID('instance', 'Test2'), article),
    ]
    ...
    expect(errors).toEqual([
      {
        ...
        detailedMessage:
          'More than one translation found for locales en-us. Only one translation per locale is supported.',
      },
    ])
  })
