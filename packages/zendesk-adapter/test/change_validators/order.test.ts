...
describe('orderInstanceContainsAllTheInstancesValidator', () => {
  ...
  const automation1 = new InstanceElement('automation1', automationType, {
    title: 'automation1',
    active: true,
    position: 1,
  })
  const automation2 = new InstanceElement('automation2', automationType, {
    title: 'automation2',
    active: true,
    position: 2,
  })
  const automation3 = new InstanceElement('automation3', automationType, {
    title: 'automation3',
    active: false,
    position: 3,
  })
  const automationOrder = new InstanceElement(ElemID.CONFIG_NAME, automationOrderType, {
    active: [
  ...
  const workspace1 = new InstanceElement('workspace1', workspaceType, {
    title: 'workspace1',
    activated: true,
    position: 1,
  })
  const workspace2 = new InstanceElement('workspace2', workspaceType, {
    title: 'workspace2',
    activated: true,
    position: 2,
  })
  const workspace3 = new InstanceElement('workspace3', workspaceType, {
    title: 'workspace3',
    activated: false,
    position: 3,
  })
  const workspaceOrder = new InstanceElement(ElemID.CONFIG_NAME, workspaceOrderType, {
    active: [
      new ReferenceExpression(workspace1.elemID, workspace1),
      new ReferenceExpression(workspace2.elemID, workspace2),
    ],
    inactive: [new ReferenceExpression(workspace3.elemID, workspace3)],
  })
  ...
  it('should return an error if the instance does not exist in the correct activity list', async () => {
    const invalidOrderInstance = new InstanceElement(ElemID.CONFIG_NAME, automationOrderType, {
      active: [
        new ReferenceExpression(automation1.elemID, automation1),
        new ReferenceExpression(automation2.elemID, automation2),
      ],
    ...
    const elementsSource = buildElementsSourceFromElements(
      [automationType, automationOrderType, automation1, automation2, automation3, invalidOrderInstance].map(e =>
        e.clone(),
      ),
    )
  ...
  it('should return an error if the instance exist on the other activity list', async () => {
    const invalidOrderInstance = new InstanceElement(ElemID.CONFIG_NAME, workspaceOrderType, {
      active: [
        new ReferenceExpression(workspace1.elemID, workspace1),
        new ReferenceExpression(workspace2.elemID, workspace2),
      ],
    ...
    const elementsSource = buildElementsSourceFromElements(
      [workspaceType, workspaceOrderType, workspace1, workspace2, workspace3, invalidOrderInstance].map(e => e.clone()),
    )
...
Please make sure to place it under the active list`,
  ...
  it('should not return an error if there is no elements source', async () => {
    ...
    const errors = await orderInstanceContainsAllTheInstancesValidator(elementsToAdd.map(e => toChange({ after: e })))
    expect(errors).toHaveLength(0)
  })
})
