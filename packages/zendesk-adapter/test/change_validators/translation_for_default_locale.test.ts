...
describe('translationForDefaultLocaleValidator', () => {
  ...
  const articleTranslationType = new ObjectType({ elemID: new ElemID(ZENDESK, articleTranslationTypename) })
  const guideLanguageSettingsType = new ObjectType({ elemID: new ElemID(ZENDESK, guideLanguageSettingsTypeName) })

  const guideLanguageSettingsInstance = new InstanceElement('instance', guideLanguageSettingsType, {
    locale: 'en-us',
    brand: 1,
  })
  const guideLanguageSettingsHe = new InstanceElement('instance', guideLanguageSettingsType, {
    locale: 'he',
    brand: 1,
  })
  ...
  const enArticleTranslationInstance = new InstanceElement('instance', articleTranslationType, {
    locale: 'en-us',
    title: 'name',
    body: 'description',
  })
  ...
  const enArticleTranslationInstanceReferenceLocale = new InstanceElement('instance', articleTranslationType, {
  ...
  it('should return an error when section does not have translation for source_locale', async () => {
    const invalidSectionInstance = replaceInstanceTypeForDeploy({
      instance: new InstanceElement('instance', sectionType, {
        id: 1,
        name: 'name',
    ...
    const errors = await translationForDefaultLocaleValidator([toChange({ after: invalidSectionInstance })])
  ...
  it('should not return an error when section has translation for source_locale', async () => {
    ...
    const errors = await translationForDefaultLocaleValidator([toChange({ after: validSectionInstance })])
    expect(errors).toHaveLength(0)
  })

  it('should not return an error when article has translation for source_locale', async () => {
    const validArticleInstance = replaceInstanceTypeForDeploy({
      instance: new InstanceElement('instance', articleType, {
        id: 1,
        name: 'name',
        description: 'description',
        source_locale: new ReferenceExpression(
    ...
    const errors = await translationForDefaultLocaleValidator([toChange({ after: validArticleInstance })])
    expect(errors).toHaveLength(0)
  })
  it('should return an error when a locale reference is unresolved', async () => {
    const validArticleInstance = replaceInstanceTypeForDeploy({
      instance: new InstanceElement('instance', articleType, {
        id: 1,
        name: 'name',
  ...
  it('should not return an error when article has translation for source_locale, locale is a reference expression', async () => {
    const validArticleInstance = replaceInstanceTypeForDeploy({
      instance: new InstanceElement('instance', articleType, {
        id: 1,
        name: 'name',
  ...
  it('should return an error when article does not have translation for source_locale, locale is a reference expression', async () => {
    ...
    const errors = await translationForDefaultLocaleValidator([toChange({ after: invalidArticleInstance })])
    expect(errors).toEqual([
      {
