...
} from '../../src/change_validators/trigger_order'
...
describe('triggerOrderInstanceContainsAllTheInstancesValidator', () => {
  ...
  const category1 = new InstanceElement('category1', triggerCategoryType, { name: 'category1' })
  const category2 = new InstanceElement('category2', triggerCategoryType, { name: 'category2' })
  const trigger1 = new InstanceElement('trigger1', triggerType, {
    title: 'trigger1',
    active: true,
    category_id: new ReferenceExpression(category1.elemID, category1),
  })
  ...
  const orderInstance = new InstanceElement(ElemID.CONFIG_NAME, triggerOrderType, {
    order: [
      {
  ...
  it('should return an error if the instance does not exist in the correct activity list', async () => {
    const invalidOrderInstance = new InstanceElement(ElemID.CONFIG_NAME, triggerOrderType, {
      order: [
        {
          category: new ReferenceExpression(category1.elemID, category1),
          active: [
...
If the order is important, please include it under the ${category1.elemID.name} category in the inactive list`,
  ...
  it('should return an error if the instance does exist in the wrong activity list in the same category', async () => {
    ...
    expect(errors).toEqual([
      createWrongPlaceErrorMessage(trigger3.elemID, createOrderTypeName(trigger3.elemID.typeName), false),
    ])
  })
  ...
  it('should not return an error if there is no order instance', async () => {
    const elementsToAdd = [trigger3.clone()]
    const elementsSource = buildElementsSourceFromElements(
      [triggerType, triggerOrderType, triggerCategoryType, category1, category2, trigger1, trigger2, trigger3].map(e =>
        e.clone(),
      ),
    )
    const errors = await triggerOrderInstanceContainsAllTheInstancesValidator(
  ...
  it('should not return an error if active field is missing and the change should not result error', async () => {
    const orderInstanceWithNoTriggers = new InstanceElement(ElemID.CONFIG_NAME, triggerOrderType, {
      order: [
        {
          category: new ReferenceExpression(category1.elemID, category1),
          inactive: [new ReferenceExpression(trigger3.elemID, trigger3)],
    ...
    const elementsSource = buildElementsSourceFromElements(
      [triggerType, triggerOrderType, triggerCategoryType, orderInstanceWithNoTriggers, trigger3].map(e => e.clone()),
    )
    const elementsToAdd = [trigger3.clone()]
    const errors = await triggerOrderInstanceContainsAllTheInstancesValidator(
      elementsToAdd.map(e => toChange({ after: e })),
