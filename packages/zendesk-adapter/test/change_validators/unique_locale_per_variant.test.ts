...
import { noDuplicateLocaleIdInDynamicContentItemValidator } from '../../src/change_validators/unique_locale_per_variant'
import { VARIANTS_FIELD_NAME, DYNAMIC_CONTENT_ITEM_VARIANT_TYPE_NAME } from '../../src/filters/dynamic_content'
...
describe('noDuplicateLocaleIdInDynamicContentItemValidator', () => {
  ...
  const variant1 = new InstanceElement('var1', variantType, {
    name: 'test1',
    content: 'abc',
    locale_id: new ReferenceExpression(english.elemID, english),
  })
  const variant2 = new InstanceElement('var2', variantType, {
    name: 'test2',
    content: 'abc',
    locale_id: new ReferenceExpression(spanish.elemID, spanish),
  })
  const item = new InstanceElement('item', itemType, {
    name: 'test1',
    [VARIANTS_FIELD_NAME]: [
  ...
  it('should return an error when we add a variant with an already existing locale', async () => {
    ...
    const errors = await noDuplicateLocaleIdInDynamicContentItemValidator([toChange({ after: clonedVariant })])
  ...
  it('should return an error if the variant does not have a parent', async () => {
    ...
    const item2 = new InstanceElement('item2', itemType, {
      name: 'test2',
      [VARIANTS_FIELD_NAME]: [new ReferenceExpression(variant2.elemID, variant2), 'some string'],
    })
