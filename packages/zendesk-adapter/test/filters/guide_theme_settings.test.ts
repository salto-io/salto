...
const liveTheme = new InstanceElement('theme', themeType,
  { id: 'park?', name: 'SixFlags', brand_id: new ReferenceExpression(brand1.elemID, brand1), live: true })
const nonLiveTheme = new InstanceElement('theme', themeType,
  { id: 'park?2', name: 'SixFlags', brand_id: new ReferenceExpression(brand1.elemID, brand1), live: false })
...
const liveTheme2 = new InstanceElement('theme2', themeType,
  { id: 'park?3', name: 'SixFlags', brand_id: new ReferenceExpression(brand2.elemID, brand2), live: true })
...
describe('filterCreator', () => {
  ...
  describe('deploy', () => {
    ...
    it('should fail on addition and deletion changes', async () => {
      mockPublish.mockResolvedValue([])
      const changes = [
        toChange({ before: themeSettingsInstance }),
        toChange({ after: themeSettingsInstance2 }),
      ]
      const deployResults = await filter.deploy?.(changes)
