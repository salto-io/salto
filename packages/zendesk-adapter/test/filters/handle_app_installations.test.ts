...
`,
  ...
  describe('onFetch', () => {
    ...
    it('Should map single id fields', async () => {
      await initFilterAndFetch()
      expect(app.value.settings.mapped_field).toEqual(new TemplateExpression({
        parts: [new ReferenceExpression(field1.elemID, field1)],
      }))
      expect(app.value.settings.ticketFieldMapped).toEqual(new TemplateExpression({
        parts: [new ReferenceExpression(field1.elemID, field1)],
      }))
      expect(app.value.settings.oneMapped_options).toEqual(new TemplateExpression({
        parts: ['custom_field_', new ReferenceExpression(option1.elemID, option1)],
      }))
      expect(app.value.settings.mappedGroup).toEqual(new TemplateExpression({
    ...
    it('Should map comma separated fields', async () => {
      await initFilterAndFetch()
      expect(app.value.settings.mapped_fields).toEqual(new TemplateExpression({
        parts: [
          'before',
    ...
    it('Should map newline separated fields', async () => {
      await initFilterAndFetch()
      expect(app.value.settings.mappedWithNewlineSeparatorGroups).toEqual(new TemplateExpression({
        parts: [
          '\nbefore',
    ...
    it('should map all fields with greedy applied', async () => {
      ...
      expect(app.value.settings.unmappedF).toEqual(new TemplateExpression({
        parts: [new ReferenceExpression(field2.elemID, field2)],
      }))
      expect(app.value.settings.unmappedFO).toEqual(new TemplateExpression({
        parts: [new ReferenceExpression(option2.elemID, option2)],
      }))
      expect(app.value.settings.unmappedG).toEqual(new TemplateExpression({
