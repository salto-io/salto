...
describe('handle templates filter', () => {
  ...
  const dynamicContentComplicated = new InstanceElement('dynamicContentComplicated', dynamicContentItemType, {
    id: newId(),
  ...
  describe('on fetch', () => {
    ...
    it('should resolve more complicated templates correctly', () => {
      ...
      const fetchedDynamicContentComplicated = elements
        .filter(isInstanceElement)
        .find(i => i.elemID.name === 'dynamicContentComplicated')
      expect(fetchedDynamicContentComplicated?.value.content).toEqual(
        new TemplateExpression({
          parts: [
            `content: {{${TICKET_TICKET_FIELD}_`,
            new ReferenceExpression(placeholder1.elemID, placeholder1),
