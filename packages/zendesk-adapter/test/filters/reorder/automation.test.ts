...
describe('automation reorder filter', () => {
  ...
  describe('onFetch', () => {
    it('should create correct order element', async () => {
      ...
      const automationOrderType = elements.find(e => isObjectType(e) && e.elemID.typeName === orderTypeName)
      expect(automationOrderType).toBeDefined()
      const automationOrderInstance = elements.find(e => isInstanceElement(e) && e.elemID.typeName === orderTypeName)
      expect(automationOrderInstance).toBeDefined()
      expect(automationOrderInstance?.elemID.name).toEqual(ElemID.CONFIG_NAME)
      expect((automationOrderInstance as InstanceElement)?.value).toEqual({
      ...
      const orderType = elements.find(elem => elem.elemID.getFullName() === 'zendesk.automation_order')
