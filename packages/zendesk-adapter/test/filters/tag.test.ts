  ...
  describe('with tags included', () => {
    ...
    describe('onFetch', () => {
      it('should change the tags to be array references', async () => {
        const elements = [
          tagType, slaPolicyType, triggerType, ticketFieldType, userSegmentType,
      ...
      it('should change the tags to arrays but not convert to references', async () => {
        ...
        expect(trigger?.value).toEqual({
          ...
          conditions: {
            ...
            any: [
              { field: 'current_tags', operator: 'includes', value: ['t5', 't6'] },
            ],
          },
        })
        const sla = instances.find(e => e.elemID.typeName === 'sla_policy')
        expect(sla?.value).toEqual({
          ...
          filter: {
            all: [
              { field: 'current_tags', operator: 'includes', value: ['t3'] },
            ],
            any: [
              { field: 'current_tags', operator: 'includes', value: ['t5', 't6', 't2'] },
            ],
          },
          policy_metrics: [
            {
