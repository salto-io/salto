...
*/
import { ChangeGroupId, ChangeId, ElemID, InstanceElement, ObjectType, toChange, CORE_ANNOTATIONS, Change, ReferenceExpression } from '@salto-io/adapter-api'
import { ZENDESK_SUPPORT } from '../src/constants'
import { getChangeGroupIds } from '../src/group_change'

describe('Group changes function', () => {
  const groupObjType = new ObjectType({ elemID: new ElemID(ZENDESK_SUPPORT, 'group') })
  const groupInstance1 = new InstanceElement('group1', groupObjType)
  const groupInstance2 = new InstanceElement('group2', groupObjType)
  const ticketFieldObjType = new ObjectType({ elemID: new ElemID(ZENDESK_SUPPORT, 'ticket_field') })
  const ticketFieldOptionObjType = new ObjectType({ elemID: new ElemID(ZENDESK_SUPPORT, 'ticket_field__custom_field_options') })
    ...
    ticketFieldObjType,
    {
      ...
      custom_field_options: [
        new ReferenceExpression(
          new ElemID(
            ZENDESK_SUPPORT, ticketFieldOptionObjType.elemID.typeName, 'instance', 'option1',
          ),
        ),
      ],
    },
  ...
  const ticketField2 = new InstanceElement(
    ...
    ticketFieldObjType,
    {
      ...
      custom_field_options: [
        new ReferenceExpression(
          new ElemID(
            ZENDESK_SUPPORT, ticketFieldOptionObjType.elemID.typeName, 'instance', 'option2',
          ),
        ),
      ],
    },
  ...
  describe('groups for recurse into types', () => {
    it('should have the same group id for an instance and its children', () => {
      ...
      expect(changeGroupIds.get(ticketField1.elemID.getFullName()))
        .toEqual(ticketField1.elemID.getFullName())
    })
    it('should have different group for two recurse into parents', () => {
      ...
      expect(changeGroupIds.get(ticketField1.elemID.getFullName()))
        .toEqual(ticketField1.elemID.getFullName())
      expect(changeGroupIds.get(ticketField2.elemID.getFullName()))
        .toEqual(ticketField2.elemID.getFullName())
    })
  })
})
