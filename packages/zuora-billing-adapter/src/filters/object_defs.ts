...
import {
  ...
  OBJECTS_PATH,
  METADATA_TYPE,
  STANDARD_OBJECT_DEFINITION_TYPE,
  CUSTOM_OBJECT_SUFFIX,
  LABEL,
  OBJECT_TYPE,
  FIELD_RELATIONSHIP_ANNOTATIONS,
  REQUIRED,
  FILTERABLE,
  DESCRIPTION,
  INTERNAL_ID,
...
const addRelationships = (
  ...
  const findType = (objName: string): ObjectType | undefined => typesByLowercaseName[objName.toLowerCase()]
  ...
  Object.entries(origInstancesByName)
    ...
    .forEach(([name, inst]) => {
      ...
      if (obj === undefined || !Array.isArray(relationships) || !relationships.every(_.isPlainObject)) {
        return
      }
...
const filterCreator: FilterCreator = () => ({
  ...
  onFetch: async (elements: Element[]): Promise<void> => {
    ...
    const customObjectDefType = elements.filter(isObjectType).find(e => e.elemID.name === CUSTOM_OBJECT_DEFINITION_TYPE)
    ...
    const defInstancesByName: Record<string, InstanceElement> = _.keyBy(objectDefInstances, typeName)
    ...
    const typesByLowercaseName = _.mapKeys(defInstancesObjects, (_v, k) => k.toLowerCase())
